...
void subfind_unbind_independent_ones(int count_cand)
{
  ...
  R2list = (struct r2data *) mymalloc("R2list", NumPart * sizeof(struct r2data));
  ud = (struct unbind_data *) mymalloc("ud", NumPart * sizeof(struct unbind_data));

#ifdef OMP_SORT
  omp_qsort(candidates, count_cand, sizeof(struct cand_dat), subfind_compare_candidates_nsubs);
#else
...
{
  long long p;
  int len, len_non_gas, LocalNonGasLen, totgrouplen1, totgrouplen2;
...
#ifdef OMP_SORT
  omp_qsort(P, NumPart, sizeof(struct particle_data), subfind_compare_P_GrNrGrNr);
#else
  ...
  sd = (struct sort_density_data *) mymalloc("sd", NumPartGroup * sizeof(struct sort_density_data));
...
#ifdef OMP_SORT
  omp_qsort(P, NumPart, sizeof(struct particle_data), subfind_compare_P_origindex);	/* reorder them such that the gas particles match again */
#else
  ...
  Head = (long long *) mymalloc("Head", NumPartGroup * sizeof(long long));
  Next = (long long *) mymalloc("Next", NumPartGroup * sizeof(long long));
  Tail = (long long *) mymalloc("Tail", NumPartGroup * sizeof(long long));
  Len = (int *) mymalloc("Len", NumPartGroup * sizeof(int));
  ...
  max_candidates = IMAX((NumPartGroup / 50), 200);
  candidates = (struct cand_dat *) mymalloc("candidates", max_candidates * sizeof(struct cand_dat));
  count_cand = 0;
  ...
  do
    {
      ...
      t0 = second();
      if(ThisTask == 0)
	tmp_candidates = (struct cand_dat *) mymalloc("	tmp_candidates", totcand * sizeof(struct cand_dat));

      count = count_cand;
      count *= sizeof(struct cand_dat);

      countlist = (int *) mymalloc("countlist", NTask * sizeof(int));
...
#ifdef OMP_SORT
	  omp_qsort(tmp_candidates, totcand, sizeof(struct cand_dat), subfind_compare_candidates_rank);
#else
...
#ifdef OMP_SORT
	  omp_qsort(tmp_candidates, totcand, sizeof(struct cand_dat), subfind_compare_candidates_subnr);
#else
...
#ifdef OMP_SORT
      omp_qsort(P, NumPart, sizeof(struct particle_data), subfind_compare_P_submark);	/* groups particles of the same canidate together */
#else
...
#ifdef NO_GAS_CLOUDS
	      MPI_Allreduce(&LocalNonGasLen, &len_non_gas, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
	      if(len_non_gas >= All.DesLinkNgb)
#else
	      if(len >= All.DesLinkNgb)
#endif
...
//sprintf(tag_path,"%s/tagged_col_%03d",All.OutputDir,All.SnapshotFileCount-1);
//sprintf(tag_file_name,"%s/tagged_%03d/tagged_halos_col_%03d_%03d",All.OutputDir,grindex,grindex,Nsubgroups);
//int tag_file_status=-1;
//if(ThisTask==0)
//	tag_file_status=mkdir(tag_path,02755);
//#endif
	...
	//printf(tag_file_name,"%s/tagged_%03d/tagged_halos_col_%03d_%03d_%03d",All.OutputDir,grindex,grindex,Nsubgroups,k);
//	printf(tag_file_name,"%s/tagged_%03d/tagged_halos_col_%03d_%03d_%03d",All.OutputDir,All.SnapshotFileCount,grindex,Nsubgroups,k);

//	memset(&tagged_p,0,sizeof(struct tagged_particle));
//	tagged_p.HaloIndex=subnr;
  //      tagged_p.Snap=All.SnapshotFileCount;
    //    tagged_p.Time=All.Time;
    //    tagged_p.SubhaloIndex=Nsubgroups;
    //    tagged_p.HaloIndex=k;
//	#endif
	      tt0 = second();
#ifdef LT_ADD_GAL_TO_SUB
	      subfind_col_determine_sub_halo_properties(ud, LocalLen, &SubMass,
							...
							&SubLum[0], &SubLumAtt[0], &SubLumObs[0], &Dust[0],
							&SubMeanAge, &SubMeanZStar, &SubSFR);
#else
	      subfind_col_determine_sub_halo_properties(ud, LocalLen, &SubMass,
							&SubPos[0], &SubVel[0], &SubCM[0], &SubVelDisp,
							&SubVmax, &SubVmaxRad, &SubSpin[0], &SubMostBoundID,
		  ...
		  if(head != head_attach)
		      ...
		      if(len_attach >= All.DesLinkNgb)
			{
			  if(count_cand < max_candidates)
			    ...
			    {
			      printf("Task %d: count=%d, max=%d, npartgroup=%d\n", ThisTask, count_cand,
				     max_candidates, NumPartGroup);
			      endrun(87);
			    }
			}
...
int subfind_col_unbind(struct unbind_data *d, int num, int *num_non_gas)
{
  ...
  do
      ...
      for(i = 0; i < num; i++)
	{
      ...
      nbu_count = (int *) mymalloc("nbu_count", NTask * sizeof(int));
      offset = (int *) mymalloc("offset", NTask * sizeof(int));
