...
#ifdef disk_test
void force_test_gal(int target)
{
    int ptype;
    double surfaceden, diskR, ForceDiskRad, y, ForceDiskVer, ztot, zo, z1, BHforce;
    double r2, dx, dy, dz, mass, r, a, rscale;
    double BulgeF, ad_x, ad_y, ad_z, pos_x, pos_y, pos_z;
    static double ad[3]={0.0};
    double diskmass;
    double ycheck;
    static double xmin;
    static double xmax;
    double bulgemass, diskscaler, bhmass;
    ...
    bulgemass = 2.0;
    diskmass = 5.0;
    diskscaler = 3.5;//0.003 in Mpc units
    bhmass = .00043;
    BulgeF = 0;
    ForceDiskRad = 0;
    ForceDiskVer = 0;
    ...
    dx = a * pos_x;  //Galaxy position (0,0,0)
    dy = a * pos_y;
    dz = a * pos_z;
    ...
    rscale = diskscaler * 0.1;   
    BulgeF = (bulgemass) / ( (pow(r + rscale, 2.0) + pow(0.017,2)) * r);

    
    diskR = sqrt(dx*dx + dy*dy + All.SofteningHalo * All.SofteningHalo);
    ...
    if(diskR <= 1.0)
    {
        ...
        if(ycheck  > xmin)
        {
            ...
            ForceDiskRad = (2 * All.G * surfaceden * y * (gsl_sf_bessel_I0_scaled(y) * gsl_sf_bessel_K0_scaled(y) - gsl_sf_bessel_I1_scaled(y) * gsl_sf_bessel_K1_scaled(y)));
        ...
        z1 = sqrt(abs(diskR * diskR - dx * dx));

        ztot = abs(z1 - 0.0);

        zo = 0.7;  //scale height of disk .2kpc

        ForceDiskVer = (2 * M_PI * All.G * surfaceden * exp(-diskR / diskscaler) * (1 - exp(-ztot / zo)));
        BHforce = (bhmass) / ( sqrt(pow(pow(r , 2.0) + pow(0.017,2),3.0)) * r);


        ad[0] += dx * (BulgeF + BHforce) + (dx/diskR) * ForceDiskRad;
        ad[1] += dy * (BulgeF + BHforce) + (dy/diskR) * ForceDiskRad;
        ad[2] += dz * (BulgeF + BHforce) + (ztot) * ForceDiskVer;



        P[target].g.GravAccel[0] += All.G * ad[0];
        P[target].g.GravAccel[1] += All.G * ad[1];
        P[target].g.GravAccel[2] += All.G * ad[2];
    }
...
double Miyamoto_Nagai_Rforce(double R,double z,double M,double a,double b)
{
        double u;
        u=a+sqrt(z*z+b*b);// we drop the G as we have it in the force function
        return (M*R)/pow(R*R+u*u,1.5);
}
double Miyamoto_Nagai_zforce(double R,double z, double M,double a, double b)
{
        double u;
        u=a+sqrt(z*z+b*b); // we drop the G as we have it in the force function
        return -(M*z*u)/(pow(R*R+u*u,1.5)*sqrt(z*z+b*b));
}


void force_gal(int gal, int target)
{
    int ptype;
    double surfaceden, diskR, ForceDiskRad, y, ForceDiskVer, ztot, zo, BHforce, diskz;// del:z1 I removed diskR2
    double r2, dx, dy, dz, mass, r, a, rscale, zsoft;
    double BulgeF, ad_x, ad_y, ad_z, pos_x, pos_y, pos_z;
    static double ad[3]={0.0};
    double diskmass;
    double ycheck;
    ...
    //double Fvdamp;
    double Softhalo;
    //double aa,RRd,zz0,ka,la,ma,kc,lc,mc,qua,c,expz;	
    ...
    ptype = P[target].Type;
////////////////////// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//Should erase this, I am setting center of mass manually
/*AllGal[gal].CM_Pos[0]=0;
AllGal[gal].CM_Pos[1]=0;
AllGal[gal].CM_Pos[2]=0;*/
///////////////////////////

    //printf("Gal Forces: CM_posx = %f, CM_posy = %f, CM_posz = %f\n", AllGal[gal].CM_Pos[0], AllGal[gal].CM_Pos[1], AllGal[gal].CM_Pos[2]);
//#ifdef Extrap   
    dx = a * (pos_x - AllGal[gal].CM_Pos[0]);
    dy = a * (pos_y - AllGal[gal].CM_Pos[1]);
    dz = (pos_z - AllGal[gal].CM_Pos[2]);
//#else

//    dx = a * (pos_x - AllGal[gal].Pos[0]);
//    dy = a * (pos_y - AllGal[gal].Pos[1]);
//    dz = a * (pos_z - AllGal[gal].Pos[2]);
//#endif  

    //Bulge Force calculation
    ...
    {
       rscale = AllGal[gal].NewDiskScaleRadius * 0.1 / All.Time; 
       bgemass = AllGal[gal].NewBulgeMass;
       blkholemass = AllGal[gal].NewBlackHoleMass;
    }
    else
    {
       rscale = AllGal[gal].DiskScaleRadius * 0.1 / All.Time;        //Assuming rb /rd = 0.1 bulge to disk scale ratio     //AllGal[gal].Rvir / 6.0;
       bgemass = AllGal[gal].BulgeMass;
       blkholemass = AllGal[gal].BlackHoleMass;
    }
#else
...
//#endif
	...
	//printf("FT2\n");
       //diskmass = (AllGal[gal].StellarMass - AllGal[gal].BulgeMass) + AllGal[gal].ColdGas;
	//this extra 1000 is for test. apparently code is in Mpc but we have kpc in this simulation. I should fix it in a better way
       //diskscaler =  AllGal[gal].DiskScaleRadius / All.Time;
       diskR = sqrt(dx*dx + dy*dy + Softhalo * Softhalo);   // All.SofteningHalo * All.SofteningHalo);
	...
	b=fabs(zo);
        ...
        if(ycheck  > xmin)
        {
           ...
           // ForceDiskRad = (4 *All.G* M_PI * surfaceden * (diskscaler/diskR) * y * y * (gsl_sf_bessel_I0_scaled(y) * gsl_sf_bessel_K0_scaled(y) - gsl_sf_bessel_I1_scaled(y) * gsl_sf_bessel_K1_scaled(y))) / All.HubbleParam;
	ForceDiskRad=Miyamoto_Nagai_Rforce(diskR,dz,M1,a1,b)+Miyamoto_Nagai_Rforce(diskR,dz,M2,a2,b)+Miyamoto_Nagai_Rforce(diskR,dz,M3,a3,b);
	...
	//test printf
	//printf("ForceDiskRad:%f\n",ForceDiskRad);
       //diskR2 = sqrt(dx*dx + dy*dy + Softhalo * Softhalo);   //All.SofteningHalo*All.SofteningHalo);



       diskz = sqrt(dx*dx + dy*dy + dz*dz);

       //z1 = sqrt(fabs(diskz*diskz - diskR2*diskR2));

       ztot = fabs(dz);  //z1 - AllGal[gal].Pos[2];

       /***Vertical force correction for particles within certain regimes where this particle force equation assumption failes
       These particles end up with too large of a verticle force when the cylindrical radius becomes small*******/

       //Fvcorr = 1.0;// / pow((1.0 + pow(fabs(dz) / (1.7*diskscaler),1.65)), 1.212);
	//printf("Fvcorr:%f\n",Fvcorr);
       ...
       //Fvdamp =1.0;// exp(-0.049729 / (diskR2 * diskR2));          //-1.0*pow(0.223,2.0) / pow(diskR2,2.0) ); 

       zsoft = sqrt(ztot*ztot + Softhalo * Softhalo);       //All.SofteningHalo*All.SofteningHalo);

       //zo = 0.0007 / All.Time;  //scale height of disk .2kpc
	zo =diskscaler/9.4;// 
	//in disk test
	//
	//FDiskVer = (2.0 * M_PI * All.G * surfaceden * exp(-(diskR / diskscaler)) * (1.0 - exp(-(ztot / zo))));
	//RdZd=zo*zo+diskscaler*diskscaler;
//	ForceDiskVer=fabs(4.0*M_PI*surfaceden*aa*zz0*zz0*zz0*pow(expz,0.2))/(All.HubbleParam);
	ForceDiskVer=fabs(Miyamoto_Nagai_zforce(diskR,dz,M1,a1,b)+Miyamoto_Nagai_zforce(diskR,dz,M2,a2,b)+Miyamoto_Nagai_zforce(diskR,dz,M3,a3,b));

        //convert from cgs to current unit
        //ForceDiskVer*=1.0e-3;//convert units
        //
//	FDiskVer=fabs(4.0*M_PI*All.G*surfaceden*aa*zz0*zz0*zz0*pow(expz,0.2))/(All.HubbleParam);
	//test printf
	//printf("dR/ds:%f-%f\n",diskR,diskscaler);
	//printf("FDiskVer1:%f\n",FDiskVer);
       //FDiskVer = fabs((2 * M_PI  * surfaceden * exp(-diskR2 / diskscaler) * ztot * (1 - exp(-zsoft / zo))) /  ( zsoft * All.HubbleParam));
	//FDiskVer =1.0 - exp(-(ztot/zo));
       if(dz < 0.0)
       {
          dirz = 1.0;
       }
       else
      {
        ...
        //FDiskVer *= dirz;
	//ForceDiskRad*=expz;
	//        //ForceDiskVer=FDiskVer* Fvcorr * Fvdamp;
        FDiskVer=ForceDiskVer* dirz;
        //printf("ForceDiskVer:%f\n",ForceDiskVer);
        //printf("ForceDiskRad:%f\n",ForceDiskRad);
	//test printf
	//printf("FDiskVer2:%f\n",FDiskVer);
	//printf("DRSC:%f\n",exp(-diskR/diskscaler));
	//printf("All.G:%f\n",All.G);
	//printf("-ztot/zo:%f\n",-ztot/zo);
	//printf("expztot:%f\n",exp(-(ztot/zo)));
	//printf("ztot:%f\n",ztot);
	...
	//printf("diskmass=%f\n",diskmass);
       ...
       BHforce = (r * blkholemass) / ( All.HubbleParam * sqrt(pow((r*r + Softhalo*Softhalo), 3.0)));
       //BHforce = (r * blkholemass) / ( All.HubbleParam * sqrt(pow(pow(r , 2.0) + pow(Softhalo,2),3.0)));
      
#ifdef Disk_Orientation
	double DiskRad, DiskRadx, DiskRady;
	DiskRadx = (dx / diskR) *ForceDiskRad;
	DiskRady = (dy/ diskR) *ForceDiskRad;
//void Rotate_Forces(double Diskz, double Diskx, double Disky, int gal)
	Rotate_Forces(ForceDiskVer,DiskRadx,DiskRady,gal);
       ad[0] += (dx/r) * (BulgeF + BHforce) + DiskF.Fx;
       ad[1] += (dy/r) * (BulgeF + BHforce) + DiskF.Fy;
       ad[2] += -1.0 * (dz/r) * (BulgeF + BHforce) + DiskF.Fz;
#else

        ad[0] += (dx/r) * (BulgeF + BHforce) + (dx/diskR) *  ForceDiskRad;
        ad[1] += (dy/r) * (BulgeF + BHforce) +  (dy/diskR) * ForceDiskRad;
        ad[2] += -1.0 * (dz/r) * (BulgeF + BHforce) + FDiskVer ;
	//ad[2] += -1.0 * (dz/r) * (BulgeF + BHforce) + (FDiskVer * Fvcorr * Fvdamp);
	//test printf
	//ForceDiskVer=FDiskVer* Fvcorr * Fvdamp;
//	ForceDiskVer=FDiskVer* dirz;
//	printf("ForceDiskVer:%f\n",ForceDiskVer);
//	printf("ForceDiskRad:%f\n",ForceDiskRad);
...
void potential_gal(int gal, int target)
{
    int ptype;
    double r2, dx, dy, dz, mass, r, a, rscale;
   ...
   // printf("gal = %d, target = %d\n", gal, target);
    ...
    rscale = AllGal[gal].Rvir / 6.0;
        
    pdisk = (mass * AllGal[gal].Mvir) / ((r + rscale + pow(0.017, 2)) * All.HubbleParam);
    
   // if(ThisTask == 0)
    //{
      //  printf("r2 = %f, pdisk = %f, part ID = %d\n", r2, pdisk, P[target].ID);
   // }
/* store result at the proper place */
    //P[target].p.Potential = pdisk;

    return;

}

double *force_disk(int target, int mode)
{
    int i, ptype;
    double r2, dx, dy, dz, mass, r, a, rscale;
    double adisk, ad_x, ad_y, ad_z, pos_x, pos_y, pos_z;
    static double ad[3]={0.0};
   ...
   /* if(ThisTask == 1)
    {
         test = mass * AllGal[2500].Mvir;
         printf("Testing Mass %f\n", test);
    }*/
    for(i=0; i<NumGalaxies; i++)
    {

        dx = a * (pos_x - AllGal[i].Pos[0]);
        dy = a * (pos_y - AllGal[i].Pos[1]);
        dz = a * (pos_z - AllGal[i].Pos[2]);

        r2 = dx * dx + dy * dy + dz * dz;

        r = sqrt(r2);

        rscale = AllGal[i].Rvir / 6.0;
        
        if(r > 0)
        {     
            adisk = (mass * AllGal[i].Mvir) / ( (pow(r + rscale, 2.0) + pow(0.017,2)) * r * All.HubbleParam);
...
double potential_disk(int target, int mode)
{
     int i, ptype;
     double r2, dx, dy, dz, r, rscale;
     double pos_x, pos_y, pos_z;
     static double pdisk;
     float mass = 1.0;  //1.0e10;
     ...
     {  
          ...
          dx = pos_x - AllGal[i].Pos[0];
          dy = pos_y - AllGal[i].Pos[1];
          dz = pos_z - AllGal[i].Pos[2];;
          ...
          if(r>0)
          {
               pdisk += (mass * AllGal[i].Mvir) / ((r + rscale + pow(0.017, 2)) * All.HubbleParam);
          }
     }
				  ...
				  int *exportindex)
{
  ...
  /*if(ThisTask == 0)
  {
    printf("target = %d, rcut = %f, asmth = %f\n",target, rcut, asmth);
  }*/
  ...
  /*if(ThisTask == 0)
  {
    printf("rcut2 = %f, asmthfac = %f, posx = %f, posy = %f, posz = %f\n", rcut2, asmthfac, pos_x, pos_y, pos_z);
  }*/
#if !defined(UNEQUALSOFTENINGS) && !defined(ADAPTGRAVSOFT)
  h = All.ForceSoftening[ptype];
  h_inv = 1.0 / h;
