...
/* DISTORTION PARTICLE SETUP */
  ...
  for(i = 0; i < N_gas; i++)	/* initialize sph_properties */
	...
	SphP[i].DelayTime = 0;
      ...
      SphP[i].eta2 =
	-(All.Eta2Max -
	  All.Eta2Min) / 2. * cos((P[i].Pos[2] - LONG_Z / 2) * M_PI / (2 * All.Alfa2H)) * OmegaR(i,
												 -1) +
	(All.Eta2Max + All.Eta2Min) / 2.;
      if(fabs(P[i].Pos[2] - LONG_Z / 2) > (2 * All.Alfa2H))
	SphP[i].eta2 = All.Eta2Max;
      if((P[i].Pos[0] - LONG_Y / 2.) * (P[i].Pos[0] - LONG_Y / 2.) +
	 (P[i].Pos[1] - LONG_X / 2) * (P[i].Pos[1] - LONG_X / 2.) > 8 * 8)
	SphP[i].eta2 = All.Eta2Max;
#endif
#ifdef FS_TURB_ESTIM
      if(P[i].Type == 0)
      ...
      else
	{
	  if(All.BiniX != 0 && (All.BiniY != 0 || All.BiniZ != 0))
	    {
	      ...
	      if(ThisTask == 0)
		printf
		  ("Initializing EulerPotentials from current Bini settings not yet supported for PERIODIC\n");
	      endrun(6519654);
#endif
	    }
	  else
      ...
      SphP[i].EulerA = (a0 * cos(fac * P[i].Pos[0]) +
			a1 * cos(fac * P[i].Pos[1]) + a2 * cos(fac * P[i].Pos[2])) / fac * sqrt(3);
      SphP[i].EulerB = (b0 * sin(fac * P[i].Pos[0]) +
			b1 * sin(fac * P[i].Pos[1]) + b2 * sin(fac * P[i].Pos[2])) / fac * sqrt(3);
#else
  ...
  for(i = 0; i < N_gas; i++)	/* initialize sph_properties */
      ...
      if(header.flag_entropy_instead_u == 0)
	{
	  ...
	  eos_calc_egiven(SphP[i].d.Density * All.UnitDensity_in_cgs, SphP[i].xnuc, SphP[i].EntropyPred,
			  &SphP[i].temp, &res);
	  SphP[i].Pressure = res.p.v / All.UnitPressure_in_cgs;
	  // Warning: dpdr is in physical units ...
	  SphP[i].dpdr =
	    res.p.drho +
	    res.temp * gsl_pow_2(res.p.dtemp / (SphP[i].d.Density * All.UnitDensity_in_cgs)) / res.e.dtemp;
#endif
	}

#if defined (VS_TURB) || defined (AB_TURB)
      SphP[i].Entropy = SphP[i].EntropyPred = All.RefEntropy;
#endif
