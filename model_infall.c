...
void strip_from_satellite(int halonr, int centralgal, int gal)
{
  ...
  if(ReionizationOn)
    reionization_modifier = do_reionization(gal, ZZ[Halo_s[halonr].SnapNum]);
  else
    reionization_modifier = 1.0;
...
double do_reionization(int gal, double Zcurr)
{
  ...
  omegaZ = Omega * (pow(1.0 + Zcurr, 3.0) / (Omega * pow(1.0 + Zcurr, 3.0) + OmegaLambda_s));
  xZ = omegaZ - 1.0;
  deltacritZ = 18.0 * M_PI * M_PI + 82.0 * xZ - 39.0 * xZ * xZ;
  HubbleZ = All.Hubble * sqrt(Omega * pow(1.0 + Zcurr, 3.0) + OmegaLambda_s);

  Mchar = Vchar * Vchar * Vchar / (All.G * HubbleZ * sqrt(0.5 * deltacritZ));
