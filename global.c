#include <mpi.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>

#include "allvars.h"
#include "proto.h"
#ifdef VORONOI
#include "voronoi.h"
#endif


/* This routine computes various global properties of the particle
 * distribution and stores the result in the struct `SysState'.
 * Currently, not all the information that's computed here is 
 * actually used (e.g. momentum is not really used anywhere),
 * just the energies are written to a log-file every once in a while.
 */
void compute_global_quantities_of_system(void)
{
  int i, j, n, dt_step;
  struct state_of_system sys;
  double a1, a2, a3;
  double entr = 0, egyspec, vel[3];
  double dt_entr, dt_gravkick, dt_hydrokick;
#if defined(FS_ALFA2_TURB)
  struct
  {
    double val;
    int where;
  } ain, aout;
#endif
#if defined(MAGNETIC_STATISTICS) || defined(FS_ALFA2_QUENCH)
  double dt_mag, mag[3];
  mag[0] = mag[1] = mag[2] = 0.0;
  sys.EnergyMag = 0.0;
#if defined(TRACEDIVB)
  sys.DivBerr = 0;
#endif
#ifdef FS_ALFA2_QUENCH
  sys.MaxAlfa2 = 0;
#endif
#if defined(FS_ALFA2_TURB)
  sys.DensestPart[3] = 0.0;
#endif
#endif

#if defined(AB_SHOCK_STATISTICS) || (defined(MAGNETIC_STATISTICS) && defined(TRACEDIVB))
  double volweight = 0, massweight = 0, Volweight = 0, Massweight = 0;
#endif
#ifdef AB_SHOCK_STATISTICS
  sys.MachHydroMass = sys.MachHydroVol = 0.0;
#if defined(MAGNETIC_STATISTICS) && !defined(AB_SHOCK_MAGNETIC)
  sys.MachMagMass = sys.MachMagVol = 0.0;
#endif
#endif

  if(All.ComovingIntegrationOn)
    {
      a1 = All.Time;
      a2 = All.Time * All.Time;
      a3 = All.Time * All.Time * All.Time;
    }
  else
    {
      a1 = a2 = a3 = 1;
    }


  for(n = 0; n < 6; n++)
    {
      sys.MassComp[n] = sys.EnergyKinComp[n] = sys.EnergyPotComp[n] = sys.EnergyIntComp[n] = 0;

      for(j = 0; j < 4; j++)
	sys.CenterOfMassComp[n][j] = sys.MomentumComp[n][j] = sys.AngMomentumComp[n][j] = 0;
    }

  for(i = 0; i < NumPart; i++)
    {
      sys.MassComp[P[i].Type] += P[i].Mass;

#if defined(EVALPOTENTIAL) || defined(COMPUTE_POTENTIAL_ENERGY)
      sys.EnergyPotComp[P[i].Type] += 0.5 * P[i].Mass * P[i].p.Potential / a1;
#endif

#ifndef WAKEUP
      dt_step = P[i].TimeBin ? (((integertime) 1) << P[i].TimeBin) : 0;
#else
      dt_step = P[i].dt_step;
#endif

      if(All.ComovingIntegrationOn)
	{
	  dt_entr = (All.Ti_Current - (P[i].Ti_begstep + dt_step / 2)) * All.Timebase_interval;
	  dt_gravkick = get_gravkick_factor(P[i].Ti_begstep, All.Ti_Current) -
	    get_gravkick_factor(P[i].Ti_begstep, P[i].Ti_begstep + dt_step / 2);
	  dt_hydrokick = get_hydrokick_factor(P[i].Ti_begstep, All.Ti_Current) -
	    get_hydrokick_factor(P[i].Ti_begstep, P[i].Ti_begstep + dt_step / 2);
#if defined(MAGNETIC_STATISTICS) || defined(FS_ALFA2_QUENCH)
	  dt_mag = get_magkick_factor(P[i].Ti_begstep, All.Ti_Current) -
	    get_magkick_factor(P[i].Ti_begstep, P[i].Ti_begstep + dt_step / 2);
#endif
	}
      else
	dt_entr = dt_gravkick = dt_hydrokick =
#if defined(MAGNETIC_STATISTICS) || defined(FS_ALFA2_QUENCH)
	  dt_mag =
#endif
	  (All.Ti_Current - (P[i].Ti_begstep + dt_step / 2)) * All.Timebase_interval;

      for(j = 0; j < 3; j++)
	{
	  vel[j] = P[i].Vel[j] + P[i].g.GravAccel[j] * dt_gravkick;
	  if(P[i].Type == 0)
	    vel[j] += SphP[i].a.HydroAccel[j] * dt_hydrokick;
	}
      if(P[i].Type == 0)
	{
	  entr = SphP[i].Entropy + SphP[i].e.DtEntropy * dt_entr;
#if defined(MAGNETIC_STATISTICS) || defined(FS_ALFA2_QUENCH)
	  for(j = 0; j < 3; j++)
	    mag[j] = SphP[i].b2.BPred[j] + SphP[i].DtB[j] * dt_mag;
#endif
	}

#ifdef PMGRID
      if(All.ComovingIntegrationOn)
	dt_gravkick = get_gravkick_factor(All.PM_Ti_begstep, All.Ti_Current) -
	  get_gravkick_factor(All.PM_Ti_begstep, (All.PM_Ti_begstep + All.PM_Ti_endstep) / 2);
      else
	dt_gravkick = (All.Ti_Current - (All.PM_Ti_begstep + All.PM_Ti_endstep) / 2) * All.Timebase_interval;

      for(j = 0; j < 3; j++)
	vel[j] += P[i].GravPM[j] * dt_gravkick;
#endif

      sys.EnergyKinComp[P[i].Type] +=
	0.5 * P[i].Mass * (vel[0] * vel[0] + vel[1] * vel[1] + vel[2] * vel[2]) / a2;

      if(P[i].Type == 0)
	{
#ifndef EOS_DEGENERATE
#if defined(TRADITIONAL_SPH_FORMULATION) || defined(ISOTHERM_EQS)
	  egyspec = entr;
#else
	  egyspec = entr / (GAMMA_MINUS1) * pow(SphP[i].d.Density / a3, GAMMA_MINUS1);
#endif
#else
	  egyspec = SphP[i].u;
#endif

#ifdef VORONOI_SHAPESCHEME
	  double dx = P[i].Pos[0] - SphP[i].Center[0];
	  double dy = P[i].Pos[1] - SphP[i].Center[1];
	  double dz = P[i].Pos[2] - SphP[i].Center[2];
	  double d2 = dx * dx + dy * dy + dz * dz;

	  double square = 1 + All.VoronoiStiffNess * d2 / pow(SphP[i].Volume, 2.0 / DIMS);
	  double curly = 1 + All.VoronoiRoundNess * (SphP[i].W / pow(SphP[i].Volume, 2.0 / DIMS) - SHAPE_FAC);

	  egyspec *= square * curly;
#endif

#if defined(MAGNETIC_STATISTICS) || defined(FS_ALFA2_QUENCH)
	  sys.EnergyMag +=
	    P[i].Mass / SphP[i].d.Density * MU0_1 / 2.0 * (mag[0] * mag[0] + mag[1] * mag[1] +
							   mag[2] * mag[2]);
#endif
#if defined(MAGNETIC_STATISTICS) && defined(TRACEDIVB)
	  if(P[i].ID != 0
	     && (SphP[i].b2.BPred[0] * SphP[i].b2.BPred[0] + SphP[i].b2.BPred[1] * SphP[i].b2.BPred[1] +
		 SphP[i].b2.BPred[2] * SphP[i].b2.BPred[2]) != 0)
	    sys.DivBerr +=
	      fabs(SphP[i].divB) * PPP[i].Hsml * P[i].Mass / (SphP[i].b2.BPred[0] * SphP[i].b2.BPred[0] +
							      SphP[i].b2.BPred[1] * SphP[i].b2.BPred[1] +
							      SphP[i].b2.BPred[2] * SphP[i].b2.BPred[2] +
							      1E-2) / SphP[i].d.Density;
#endif
#ifdef FS_ALFA2_QUENCH
	  if(fabs(SphP[i].alfa2) > sys.MaxAlfa2)
	    sys.MaxAlfa2 = fabs(SphP[i].alfa2);
#endif
#if defined(FS_ALFA2_TURB)
	  if(SphP[i].d.Density > sys.DensestPart[3])
	    {
	      sys.DensestPart[3] = SphP[i].d.Density;
	      sys.DensestPart[2] = P[i].Pos[2];
	      sys.DensestPart[1] = P[i].Pos[1];
	      sys.DensestPart[0] = P[i].Pos[0];
	    };
#endif
#if defined(AB_SHOCK_STATISTICS) || (defined(MAGNETIC_STATISTICS) && defined(TRACEDIVB))
	  volweight += P[i].Mass / SphP[i].d.Density;
	  massweight += P[i].Mass;
#endif
#if defined(AB_SHOCK_STATISTICS)
#if defined(AB_SHOCK_STATISTICS_SIMPLE) || !defined(AB_SHOCK)
#ifndef EOS_DEGENERATE
	  double sound_i2 = GAMMA * SphP[i].Pressure / SphP[i].d.Density;
#else
	  double sound_i2 = SphP[j].dpdr;
#endif
	  double v_i2 = vel[0] * vel[0] + vel[1] * vel[1] + vel[2] * vel[2];

	  sys.MachHydroMass += P[i].Mass * v_i2 / sound_i2;
	  sys.MachHydroVol += P[i].Mass / SphP[i].d.Density * v_i2 / sound_i2;
#else
	  sys.MachHydroMass += P[i].Mass * SphP[i].Shock_Mach * SphP[i].Shock_Mach;
	  sys.MachHydroVol += P[i].Mass / SphP[i].d.Density * SphP[i].Shock_Mach * SphP[i].Shock_Mach;
#endif

#if defined(MAGNETIC_STATISTICS) && !defined(AB_SHOCK_MAGNETIC)
	  double alfven_i2 =
	    (mag[0] * mag[0] + mag[1] * mag[1] + mag[2] * mag[2]) * MU0_1 / SphP[i].d.Density;

	  sys.MachMagMass += P[i].Mass * v_i2 / alfven_i2;
	  sys.MachMagVol += P[i].Mass / SphP[i].d.Density * v_i2 / alfven_i2;
#endif
#endif /* AB_SHOCK_STATISTICS */

	  sys.EnergyIntComp[0] += P[i].Mass * egyspec;
	}



      for(j = 0; j < 3; j++)
	{
	  sys.MomentumComp[P[i].Type][j] += P[i].Mass * vel[j];
	  sys.CenterOfMassComp[P[i].Type][j] += P[i].Mass * P[i].Pos[j];
	}

      sys.AngMomentumComp[P[i].Type][0] += P[i].Mass * (P[i].Pos[1] * vel[2] - P[i].Pos[2] * vel[1]);
      sys.AngMomentumComp[P[i].Type][1] += P[i].Mass * (P[i].Pos[2] * vel[0] - P[i].Pos[0] * vel[2]);
      sys.AngMomentumComp[P[i].Type][2] += P[i].Mass * (P[i].Pos[0] * vel[1] - P[i].Pos[1] * vel[0]);
    }


  /* some the stuff over all processors */
  MPI_Reduce(&sys.MassComp[0], &SysState.MassComp[0], 6, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&sys.EnergyPotComp[0], &SysState.EnergyPotComp[0], 6, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&sys.EnergyIntComp[0], &SysState.EnergyIntComp[0], 6, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&sys.EnergyKinComp[0], &SysState.EnergyKinComp[0], 6, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&sys.MomentumComp[0][0], &SysState.MomentumComp[0][0], 6 * 4, MPI_DOUBLE, MPI_SUM, 0,
	     MPI_COMM_WORLD);
  MPI_Reduce(&sys.AngMomentumComp[0][0], &SysState.AngMomentumComp[0][0], 6 * 4, MPI_DOUBLE, MPI_SUM, 0,
	     MPI_COMM_WORLD);
  MPI_Reduce(&sys.CenterOfMassComp[0][0], &SysState.CenterOfMassComp[0][0], 6 * 4, MPI_DOUBLE, MPI_SUM, 0,
	     MPI_COMM_WORLD);

#if defined(MAGNETIC_STATISTICS) || defined(FS_ALFA2_QUENCH)
  MPI_Reduce(&sys.EnergyMag, &SysState.EnergyMag, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
#endif
#if defined(FS_ALFA2_TURB)
  ain.val = sys.DensestPart[3];
  ain.where = ThisTask;
  MPI_Allreduce(&ain, &aout, 1, MPI_DOUBLE_INT, MPI_MAXLOC, MPI_COMM_WORLD);
  if(ThisTask == aout.where)
    {
      SysState.DensestPart[0] = sys.DensestPart[0];
      SysState.DensestPart[1] = sys.DensestPart[1];
      SysState.DensestPart[2] = sys.DensestPart[2];
      SysState.DensestPart[3] = sys.DensestPart[3];
    }
  MPI_Bcast(&SysState.DensestPart[0], 4, MPI_DOUBLE, aout.where, MPI_COMM_WORLD);
#endif
#if defined(FS_ALFA2_QUENCH)
  MPI_Reduce(&sys.MaxAlfa2, &SysState.MaxAlfa2, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
#endif
#if defined(MAGNETIC_STATISTICS) && defined(TRACEDIVB)
  MPI_Reduce(&sys.DivBerr, &SysState.DivBerr, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
#endif

#if defined(AB_SHOCK_STATISTICS) || (defined(MAGNETIC_STATISTICS) && defined(TRACEDIVB))
  MPI_Reduce(&volweight, &Volweight, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&massweight, &Massweight, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
#endif
#if defined(AB_SHOCK_STATISTICS)
  MPI_Reduce(&sys.MachHydroMass, &SysState.MachHydroMass, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&sys.MachHydroVol, &SysState.MachHydroVol, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
#if defined(MAGNETIC_STATISTICS) && !defined(AB_SHOCK_MAGNETIC)
  MPI_Reduce(&sys.MachMagMass, &SysState.MachMagMass, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&sys.MachMagVol, &SysState.MachMagVol, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
#endif
#endif /* AB_SHOCK_STATISTICS */

  if(ThisTask == 0)
    {

#if (defined(MAGNETIC_STATISTICS) && defined(TRACEDIVB))
      SysState.DivBerr /= Volweight;
#endif
#if defined(AB_SHOCK_STATISTICS)
      SysState.MachHydroMass = sqrt(SysState.MachHydroMass / Massweight);
      SysState.MachHydroVol = sqrt(SysState.MachHydroVol / Volweight);
#if defined(MAGNETIC_STATISTICS) && !defined(AB_SHOCK_MAGNETIC)
      SysState.MachMagMass = sqrt(SysState.MachMagMass / Massweight);
      SysState.MachMagVol = sqrt(SysState.MachMagVol / Volweight);
#endif
#endif /* AB_SHOCK_STATISTICS */

      for(i = 0; i < 6; i++)
	SysState.EnergyTotComp[i] = SysState.EnergyKinComp[i] +
	  SysState.EnergyPotComp[i] + SysState.EnergyIntComp[i];

      SysState.Mass = SysState.EnergyKin = SysState.EnergyPot = SysState.EnergyInt = SysState.EnergyTot = 0;

      for(j = 0; j < 3; j++)
	SysState.Momentum[j] = SysState.AngMomentum[j] = SysState.CenterOfMass[j] = 0;

      for(i = 0; i < 6; i++)
	{
	  SysState.Mass += SysState.MassComp[i];
	  SysState.EnergyKin += SysState.EnergyKinComp[i];
	  SysState.EnergyPot += SysState.EnergyPotComp[i];
	  SysState.EnergyInt += SysState.EnergyIntComp[i];
	  SysState.EnergyTot += SysState.EnergyTotComp[i];

	  for(j = 0; j < 3; j++)
	    {
	      SysState.Momentum[j] += SysState.MomentumComp[i][j];
	      SysState.AngMomentum[j] += SysState.AngMomentumComp[i][j];
	      SysState.CenterOfMass[j] += SysState.CenterOfMassComp[i][j];
	    }
	}

      for(i = 0; i < 6; i++)
	for(j = 0; j < 3; j++)
	  if(SysState.MassComp[i] > 0)
	    SysState.CenterOfMassComp[i][j] /= SysState.MassComp[i];

      for(j = 0; j < 3; j++)
	if(SysState.Mass > 0)
	  SysState.CenterOfMass[j] /= SysState.Mass;


      for(i = 0; i < 6; i++)
	{
	  SysState.CenterOfMassComp[i][3] = SysState.MomentumComp[i][3] = SysState.AngMomentumComp[i][3] = 0;
	  for(j = 0; j < 3; j++)
	    {
	      SysState.CenterOfMassComp[i][3] +=
		SysState.CenterOfMassComp[i][j] * SysState.CenterOfMassComp[i][j];
	      SysState.MomentumComp[i][3] += SysState.MomentumComp[i][j] * SysState.MomentumComp[i][j];
	      SysState.AngMomentumComp[i][3] +=
		SysState.AngMomentumComp[i][j] * SysState.AngMomentumComp[i][j];
	    }
	  SysState.CenterOfMassComp[i][3] = sqrt(SysState.CenterOfMassComp[i][3]);
	  SysState.MomentumComp[i][3] = sqrt(SysState.MomentumComp[i][3]);
	  SysState.AngMomentumComp[i][3] = sqrt(SysState.AngMomentumComp[i][3]);
	}

      SysState.CenterOfMass[3] = SysState.Momentum[3] = SysState.AngMomentum[3] = 0;

      for(j = 0; j < 3; j++)
	{
	  SysState.CenterOfMass[3] += SysState.CenterOfMass[j] * SysState.CenterOfMass[j];
	  SysState.Momentum[3] += SysState.Momentum[j] * SysState.Momentum[j];
	  SysState.AngMomentum[3] += SysState.AngMomentum[j] * SysState.AngMomentum[j];
	}

      SysState.CenterOfMass[3] = sqrt(SysState.CenterOfMass[3]);
      SysState.Momentum[3] = sqrt(SysState.Momentum[3]);
      SysState.AngMomentum[3] = sqrt(SysState.AngMomentum[3]);
    }

  /* give everyone the result, maybe the want to do something with it */
  MPI_Bcast(&SysState, sizeof(struct state_of_system), MPI_BYTE, 0, MPI_COMM_WORLD);
}
