...
#pragma omp parallel
      ...
      for(j = 0, Nimport = 0, Recv_offset[0] = 0, Send_offset[0] = 0; j < NTask; j++)
	{
      ...
      AddSPHDataGet =
	(struct addSPHdata_in *) mymalloc("AddSPHDataGet", Nimport * sizeof(struct addSPHdata_in));
      AddSPHDataIn =
	(struct addSPHdata_in *) mymalloc("AddSPHDataIn", Nexport * sizeof(struct addSPHdata_in));
...
#pragma omp parallel
  ...
  for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
	...
	if(SphP[i].alpha < SphP[i].alphaloc)
	  ...
	  SphP[i].alpha =
	    SphP[i].alphaloc + (SphP[i].alpha -
				SphP[i].alphaloc) * exp(-NV_dt / PPP[i].Hsml * 2.0 * VISCOSITY_SUPPRESSION *
							0.5 * SphP[i].MaxSignalVel);
