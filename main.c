...
//#ifdef THIS_IS_A_COSANG_RUN
  strcpy(Sageparam, argv[2]);
#endif

/*
  char Sagepath[100] = "/home/kmccord/Cosang_v5.1/sage";
  char Sageparam[100];
  char SAGE[512];
  char lockfile[100];
  int x = 1;
  ...
  create_lock_file();
  strcpy(Sageparam, argv[3]);
  strcpy(lockfile, "rm /lustre/projects/p004_swin/kmccord/halotree_test/lock_file.txt");

  sprintf(SAGE, "%s %s", Sagepath, Sageparam);
  system(SAGE); 
  system(lockfile);*/   /* runs sage and then removes the lock file */
  /*while(x != 0) 
  {
      x = lock_file_exists();
      usleep(1000);  */ /* wait until the lock file disappears, which means that SAGE has finished */
  //}
//#endif
...
#pragma omp master
  ...
  if(argc < 2)
      ...
      if(ThisTask == 0)
	{
	  ...
	  printf("Call with <ParameterFile> <Sage ParameterFile (if using)> [<RestartFlag>] <Don't read sage> [<RestartSnapNum>]\n");
	  printf("\n");
  ...
  if(argc >= 4)
    ...
    RestartFlag = 0;
  if(argc >= 5)
    sageFlag = atoi(argv[4]);
  else
    sageFlag = 0;

  if(argc >= 6)
    RestartSnapNum = atoi(argv[5]);
  else
    RestartSnapNum = -1;
#else
  if(argc >= 3)
...
/*#else
  ...
  if(ThisTask ==0)
  {
     read_parameter_file_s(Sageparam);
  ...
  TotHalos = 0;
  snap_oldgal_count = 0;
  Particle_List_tracker = 0; 
  
  MPI_Bcast(&PartMass_s, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);

  begrun();			/* set-up run  */

/*****This is to set a global variable equal to the mass of the dm particles so the P.Mass can be reset before subfind runs*******/
  /*if(ThisTask == 0)
  {
    Mass_DMpart = P[0].Mass;
  }

  MPI_Bcast(&Mass_DMpart, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  */
 
  for (aa = 0; aa < NumPart; aa++)
  {
    P[aa].InitMass = P[aa].Mass;
  }
 
//  printf("Mass_DMpart = %f, P[0].Mass = %f\n", Mass_DMpart, P[0].Mass);
 


#ifdef THIS_IS_A_COSANG_RUN  
  if(RestartFlag != 0 && sageFlag == 0 && All.SnapshotFileCount > 2)
  {
     snap_oldgal_count = 2;  
 ...
 /* if(All.SnapshotFileCount > 2)
  {
     int i;
     for(i=0; i<All.SnapshotFileCount-1; i++)
     {
          basetree(i);
     }
