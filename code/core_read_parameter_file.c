...
void read_parameter_file_s(char *fname)
{
  FILE *fd;
  ...
  FirstFile = 0;
  LastFile = 0;  //NTask; // LastSnapShotNr;
  FilesPerSnapshot_s = 1;  //NTask;
  ...
  addr[nt] = SimulationDir_s;
  id[nt++] = STRING;

  /*strcpy(tag[nt], "FileWithOutputSnaps");
  addr[nt] = FileWithOutputSnaps_s;
  id[nt++] = STRING;

  strcpy(tag[nt], "FileWithSnapList");
  addr[nt] = FileWithSnapList_s;
  id[nt++] = STRING;*/
  ...
  if((fd = fopen(fname, "r")))
  {
    ...
    fclose(fd);
    printf("close parameter file\n");
    i = strlen(OutputDir_s);
    if(i > 0)
      if(OutputDir_s[i - 1] != '/')
      strcat(OutputDir_s, "/");
  }
