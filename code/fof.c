...
void fof_fof(int num)
{
  ...
  mass = 0;
#ifdef _OPENMP
#pragma omp parallel for reduction(+:ndm,mass)
#endif
  for(i = 0; i < NumPart; i++)
...
#pragma omp parallel for
  ...
  Group = (group_properties *) mymalloc("Group", sizeof(group_properties) * MaxNgroups);
#else
  Group =
...
#if defined(WINDS) && defined(VARIABLE_WINDS)
  if(num < 0)
...
#ifdef OMP_SORT
  omp_qsort(FOF_PList, NumPart, sizeof(struct fof_particle_list), fof_compare_FOF_PList_MinID);
#else
...
#pragma omp parallel for
...
#ifdef OMP_SORT
  omp_qsort(FOF_GList, NgroupsExt, sizeof(fof_group_list), fof_compare_FOF_GList_MinIDTask);
#else
  ...
  get_FOF_GList = (fof_group_list *) mymalloc("get_FOF_GList", nimport * sizeof(fof_group_list));
...
#ifdef OMP_SORT
  omp_qsort(FOF_GList, NgroupsExt, sizeof(fof_group_list), fof_compare_FOF_GList_MinID);
  omp_qsort(get_FOF_GList, nimport, sizeof(fof_group_list), fof_compare_FOF_GList_MinID);
#else
...
#ifdef OMP_SORT
  omp_qsort(get_FOF_GList, nimport, sizeof(fof_group_list), fof_compare_FOF_GList_MinIDTask);
  omp_qsort(FOF_GList, NgroupsExt, sizeof(fof_group_list), fof_compare_FOF_GList_MinIDTask);
#else
...
#endif
	{
#if defined(WINDS) && !defined(KD_SEED_STAR_MASS_FRACTION)
...
#ifdef OMP_SORT
  omp_qsort(Group, NgroupsExt, sizeof(group_properties), fof_compare_Group_MinIDTask);
#else
...
#ifdef OMP_SORT
  omp_qsort(Group, NgroupsExt, sizeof(group_properties), fof_compare_Group_MinID);
  omp_qsort(get_Group, nimport, sizeof(group_properties), fof_compare_Group_MinID);
#else
...
#ifdef _OPENMP
#pragma omp parallel for private(j,cm)
#endif
  for(i = 0; i < NgroupsExt; i++)
    {
...
#ifdef OMP_SORT
  omp_qsort(Group, Ngroups, sizeof(group_properties), fof_compare_Group_MinID);
#else
...
void fof_save_groups(int num)
{
...
#else
  parallel_sort(FOF_GList, NgroupsExt, sizeof(fof_group_list), fof_compare_FOF_GList_LocCountTaskDiffMinID);
#endif
...
#if defined(SUBFIND) || defined(DISKPOT)
  for(i = 0; i < NumPart; i++)
  ...
  if(ThisTask == 0)
    {
      sprintf(buf, "%sgroups_%03d", All.OutputDir, num);
      mkdir(buf, 02755);
    }
...
void fof_save_local_catalogue(int num)
{
  ...
  sprintf(fname, "%sgroups_%03d/%s_%03d.%d", All.OutputDir, num, "group_tab", num, ThisTask);
  if(!(fd = fopen(fname, "w")))
    {
  ...
  sprintf(fname, "%sgroups_%03d/%s_%03d.%d", All.OutputDir, num, "group_ids", num, ThisTask);
  if(!(fd = fopen(fname, "w")))
    {
      printf("can't open file `%s`\n", fname);
      endrun(1184);
...
void fof_make_black_holes(void)
{
  ...
  for(i = 0; i < Ngroups; i++)
    {
#ifndef KD_SEED_STAR_MASS_FRACTION
      if(Group[i].LenType[1] * massDMpart >=
	 (All.Omega0 - All.OmegaBaryon) / All.Omega0 * All.MinFoFMassForNewSeed)
#else
      if(Group[i].MassType[4] > KD_SEED_STAR_MASS_FRACTION * All.MinFoFMassForNewSeed
	 && Group[i].LenType[2] == 0)
#endif
  ...
  for(n = 0; n < nimport; n++)
...
#if defined(KD_SEED_STAR_MASS_FRACTION) && defined(LT_STELLAREVOLUTION)
      i = P[import_indices[n]].pt.MetID;
      MetP[i] = MetP[N_stars - 1];
      P[MetP[i].PID].pt.MetID = i;
      N_stars--;
#endif
	...
	All.SeedBlackHoleMass * import_fofmass[n] / (KD_SEED_STAR_MASS_FRACTION * All.MinFoFMassForNewSeed);
#else
      ...
      BPP(import_indices[n]).BH_sigma = pow(All.MinFoFMassForNewSeed / 1e4 / 2.06, 1. / 3.) * 1e3 / sqrt(3);
      BPP(import_indices[n]).BH_bmax = pow(All.MinFoFMassForNewSeed * 1e10, 1. / 3.) * 0.023433340 / 2;
      printf("Task %d: Seeded BH in environment: bmax=%e, sigma=%e\n",
	     ThisTask, BPP(import_indices[n]).BH_bmax, BPP(import_indices[n]).BH_sigma);
#endif
...
#ifdef OMP_SORT
  omp_qsort(FOF_GList, NgroupsExt, sizeof(fof_group_list), fof_compare_FOF_GList_MinIDTask_MinID);
#else
...
#ifdef OMP_SORT
  omp_qsort(required_groups, NgroupsExt, sizeof(struct group_mass_MinID), compare_group_mass_ID);
#else
...
#ifdef OMP_SORT
  omp_qsort(FOF_GList, NgroupsExt, sizeof(fof_group_list), fof_compare_FOF_GList_MinID);	/* restore original order */
#else
...
void read_fof(int num)
{
  ...
  if(ThisTask == 0)
    {
      sprintf(fname, "%sgroups_%03d/%s_%03d.%d", All.OutputDir, num, "group_tab", num, 0);
      if(!(fd = fopen(fname, "r")))
	{
  ...
  if(NTask != ntask)
	...
	printf
      ...
      Group = (group_properties *) mymalloc("Group", sizeof(group_properties) * (TotNgroups / NTask + NTask));
      ...
      if(ThisTask == 0)
	{
	  for(filenr = 0; filenr < ntask; filenr++)
	    {
	      sprintf(fname, "%sgroups_%03d/%s_%03d.%d", All.OutputDir, num, "group_tab", num, filenr);
	      if(!(fd = fopen(fname, "r")))
		{
		  printf("can't read file `%s`\n", fname);
		  endrun(11831);
	  ...
	  for(filenr = 0; filenr < ntask; filenr++)
	    {
	      sprintf(fname, "%sgroups_%03d/%s_%03d.%d", All.OutputDir, num, "group_ids", num, filenr);
	      if(!(fd = fopen(fname, "r")))
		{
		  printf("can't read file `%s`\n", fname);
		  endrun(1184132);
	      ...
	      while(nids > 0)
		{
		  ...
		  if(target == 0)
		    memcpy(&ID_list[nids_obtained[target]], &tmpID_list[stored], nsend * sizeof(fof_id_list));
		  else
		    {
...
#ifdef OMP_SORT
  omp_qsort(P, NumPart, sizeof(struct particle_data), io_compare_P_ID);
  omp_qsort(ID_list, Nids, sizeof(fof_id_list), fof_compare_ID_list_ID);
#else
...
#ifdef OMP_SORT
  omp_qsort(P, NumPart, sizeof(struct particle_data), fof_compare_P_SubNr);
#else
