...
/* CECILIA: extra check, to avoid type=1 particles to have different mass */
  ...
  int j;
  double localMetalsAmount[LT_NMetP];

  memset(localMetalsAmount, 0, LT_NMetP * sizeof(double));
  for(i = 0; i < N_gas; i++)
    {
      SphP[i].A = 0;
	...
	{
	  SphP[i].B[j] = 0;
	  localMetalsAmount[j] += SphP[i].Abundance[j] * P[i].Mass;
	}
    }

  MPI_Allreduce(localMetalsAmount, All.MetalsAmount, LT_NMetP, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
#endif
