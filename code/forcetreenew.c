...
//	printf("DiO\n");
	...
	//this extra 1000 is for test. apparently code is in Mpc but we have kpc in this simulation. I should fix it in a better way
       diskscaler = 1000.0 *  AllGal[gal].DiskScaleRadius / All.Time;
	...
	//diskR2 = sqrt(dx*dx + dy*dy + Softhalo * Softhalo);   //All.SofteningHalo*All.SofteningHalo);
       ...
       //Fvcorr = pow((1.0 + pow(fabs(dz) / (1.7*diskscaler),1.65)), 1.212);
	//printf("Fvcorr:%f\n",Fvcorr);
       ...
       //Fvdamp =exp(-0.049729 / (diskR2 * diskR2));          //-1.0*pow(0.223,2.0) / pow(diskR2,2.0) ); 
	...
	////////////////// This is new force function exp(lin)=1, exp(quad)=2
	RRd=diskR/diskscaler;
	zz0=zsoft/zo;
	//ka=122.14323363; //1 
	//la=1.84; // 1
	ka=0.08266941; //2
	...
	lc=13.5884705;
	mc=4.3914054;
	//aa=ka*exp(-la*(RRd))/1000.0;// 1 this is to convert kpc to Mpc
	aa=exp(ka*(RRd-la)*(RRd-la)+ma*RRd);
	aa/=1000; // to convert from Mpc to kpc
	qua=(kc*RRd)-lc;
	c=sqrt(qua*qua+mc*(RRd));
	expz=exp(-c*zz0);
	// new verF
	...
	//FDiskVer =1.0 - exp(-(ztot/zo));
       ...
       else
      {
        ...
        FDiskVer *= dirz;
	ForceDiskRad*=expz; // new correction in Radial force
