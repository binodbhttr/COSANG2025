...
//		  printf("COM64\n");
 	 for(j=0;j<NumGalaxies;j++)
 	 {
		...
		float cmx=pcmx/cmass; //@Binod: can be divide by zero
		float cmy=pcmy/cmass; //@Binod: can be divide by zero
		float cmz=pcmz/cmass; //@Binod: can be divide by zero
	     if (AllGal[j].sub_len >= 1000) {
            if (cmass > 0.0) {
				
				//printf("This is cmass=%f\n",cmass);
				//printf("Galaxy %d: CMx=%f, CMy=%f, CMz=%f, HaloX=%f, HaloY=%f, HaloZ=%f, Mass=%f\n", j, cmx, cmy, cmz, AllGal[j].Pos[0], AllGal[j].Pos[1], AllGal[j].Pos[2], cmass);
...
int cm_iterate(const hash_t* hashtable,const hash_t* Phash, int galid, int itcount)
{
  ...
  float CM_Px_new, CM_Py_new, CM_Pz_new, CM_M_new;
  int diff;
  CM_Px = 0;
  CM_Py = 0;
  CM_Pz = 0;
				...
				if((gidx = hash_lookup(hashtable, P[i].ID)) != HASH_INVALID)
				{
					...
					//Check if particle is within shrinking sphere radius
						...
						if(rcheck <= (rsmall * rsmall) )
						{
							...
							CM_Px_new += P[partidx].Mass * P[partidx].Pos[0];
            				CM_Py_new += P[partidx].Mass * P[partidx].Pos[1];
            				CM_Pz_new += P[partidx].Mass * P[partidx].Pos[2];
            				CM_M_new  += P[partidx].Mass;
							//printf("CM_Px_new = %f\n", CM_Px_new);
							//printf("CM_Py_new = %f\n", CM_Py_new);
							//printf("CM_Pz_new = %f\n", CM_Pz_new);
							//printf("CM_M_new = %f\n", CM_M_new);
  ...
  // First processor gathers and compare and return 
	...
	if (AllGal[galid].CM_Pos[1] !=AllGal[galid].CM_Pos[1])
                cmy_orig = AllGal[i].Pos[1]; //@Binod: should be cmy_orig ?
        else
                cmy_orig = AllGal[galid].CM_Pos[1]; //@Binod: should be cmy_orig?
	if (AllGal[galid].CM_Pos[2] !=AllGal[galid].CM_Pos[2])
                cmz_orig = AllGal[i].Pos[2]; //should be cmz_orig
        else
                cmz_orig = AllGal[galid].CM_Pos[2]; //@Binod: should be cmz_orig?
