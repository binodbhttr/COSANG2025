...
double cooling_recipe(int gal, double dt)
{
  ...
  if(Gal[gal].HotGas > 1.0e-6)
  {
    ...
    x /= (All.UnitDensity_in_cgs * All.UnitTime_in_s);         // now in internal units 
    ...
    if(ThisTask == 0)
    {
      printf("x = %f, rho_rcool = %f, rho0 = %f, rcool = %f\n", x, rho_rcool, rho0, rcool);
    }
  ...
  //printf("coolingGas = %f, Rvir = %f, Vvir = %f, HotGas = %f, rcool = %f, dt = %f\n", coolingGas, Gal[gal].Rvir, Gal[gal].Vvir, Gal[gal].HotGas, rcool, dt); 
  return coolingGas;
...
double do_AGN_heating(double coolingGas, int centralgal, double dt, double x, double rcool)
{
  ...
  if(Gal[centralgal].HotGas > 0.0)
  {
    ...
    if(AGNrecipeOn == 2)
    {
      ...
      AGNrate = (2.5 * M_PI * All.G) * (0.375 * 0.6 * x) * Gal[centralgal].BlackHoleMass * RadioModeEfficiency;
    }
    ...
    else
    {
      ...
      AGNrate = RadioModeEfficiency / (All.UnitMass_in_g / All.UnitTime_in_s * SEC_PER_YEAR / SOLAR_MASS)
        * (Gal[centralgal].BlackHoleMass / 0.01) * pow(Gal[centralgal].Vvir / 200.0, 3.0)
        * ((Gal[centralgal].HotGas / Gal[centralgal].Mvir) / 0.1);
    }
    
    // Eddington rate 
    EDDrate = 1.3e48 * Gal[centralgal].BlackHoleMass / (All.UnitEnergy_in_cgs / All.UnitTime_in_s) / 9e10;
...
void cool_gas_onto_galaxy(int centralgal, double coolingGas)
{
  ...
  if(coolingGas > 0.0)
  {
    if(coolingGas < Gal[centralgal].HotGas)
    {
      ...
      if(ThisTask == 0)
      {
        printf("CoolingGas: coolingGas = %f, ColdGas = %f\n", coolingGas, Gal[centralgal].ColdGas);
      }
    ...
    else
    {
      ...
      if(ThisTask == 0)
      {
        printf("HOTGAS: HotGas = %f, ColdGas = %f\n", Gal[centralgal].HotGas, Gal[centralgal].ColdGas);
      }
