...
//		  printf("COM64\n");
 	 for(j=0;j<NumGalaxies;j++)
 	 {
		...
		float cmz=pcmz/cmass; //@Binod: can be divide by zero
	     ...
	     {
		if (cmx !=cmx)
                	AllGal[j].CM_Pos[0] = AllGal[i].Pos[0]; //@Binod should be j?
        	else
                	AllGal[j].CM_Pos[0]=cmx;
        	if (cmy !=cmy)
                	AllGal[j].CM_Pos[1]=AllGal[i].Pos[1]; //@Binod should be j?
        	else
                	AllGal[j].CM_Pos[1]=cmy;
        	if (cmz !=cmz)
                	AllGal[j].CM_Pos[2]= AllGal[i].Pos[2]; //@Binod should be j?
        	else
...
int cm_iterate(const hash_t* hashtable,const hash_t* Phash, int galid, int itcount)
{
  ...
  // First processor gathers and compare and return 
	...
	if (AllGal[galid].CM_Pos[1] !=AllGal[galid].CM_Pos[1])
                cmx_orig = AllGal[i].Pos[1]; //@Binod: should be cmy_orig ?
        else
                cmx_orig = AllGal[galid].CM_Pos[1]; //@Binod: should be cmy_orig?
	if (AllGal[galid].CM_Pos[2] !=AllGal[galid].CM_Pos[2])
                cmx_orig = AllGal[i].Pos[2]; //should be cmz_orig
        else
                cmx_orig = AllGal[galid].CM_Pos[2]; //@Binod: should be cmz_orig?
