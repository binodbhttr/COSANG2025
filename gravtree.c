...
#ifdef THIS_IS_A_COSANG_RUN 
if(All.SnapshotFileCount > 2 && TotHalos != 0 || (NumGalaxies != 0 && All.Ti_Current != 0))
{
int jj;
   #ifdef COSANG_V2
    printf("Running CoSANG V2\n");
    int flag=0;// bool needs header stdbool.h
    int gal,count, count_tot;
    int iteration;
    double Rlim;
    double temp_cm_x, temp_cm_y, temp_cm_z, temp_cm_x_tot, temp_cm_y_tot, temp_cm_z_tot,cm_x,cm_y,cm_z,cm_r,G_pos_r;
    double dx2, dy2, dz2, r2;
    double error=0.01;
//first lets find center of mass for each galaxy
    for(gal=0;gal<NumGalaxies;gal++)
	{
	#ifdef ACCURATE_COM
        count=0;
	iteration=1;
	Rlim=AllGal[gal].Rvir;
	while(flag && iteration<3)
	{
        for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
		{
		dx2=P[i].Pos[0]- AllGal[gal].Pos[0];
		dy2=P[i].Pos[1]- AllGal[gal].Pos[1];
		dz2=P[i].Pos[2]- AllGal[gal].Pos[2];
		...
		MPI_Allreduce(&temp_cm_x, &temp_cm_x_tot, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		MPI_Allreduce(&temp_cm_y, &temp_cm_y_tot, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		MPI_Allreduce(&temp_cm_z, &temp_cm_z_tot, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		MPI_Allreduce(&count, &count_tot, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	//cm_x=xFrom_Gather/count_FromGather;
	cm_x=temp_cm_x_tot/count_tot;
	cm_y=temp_cm_y_tot/count_tot;
	cm_z=temp_cm_z_tot/count_tot;
	cm_r=sqrt(cm_x*cm_x+cm_y*cm_y+cm_z*cm_z);
	G_pos_r=sqrt(AllGal[i].CM_Pos[0]*AllGal[i].CM_Pos[0]+AllGal[i].CM_Pos[1]*AllGal[i].CM_Pos[1]+AllGal[i].CM_Pos[2]*AllGal[i].CM_Pos[2]);
	if((abs((float)cm_r - G_pos_r)/G_pos_r)>error)
		{
		Rlim/=5;
		iteration++;
		}
		...
		flag=0;
	AllGal[gal].CM_Pos[0]=cm_x;
	AllGal[gal].CM_Pos[1]=cm_y;
	AllGal[gal].CM_Pos[2]=cm_z;
	}//while close
	#else
	AllGal[gal].CM_Pos[0]=AllGal[gal].Pos[0];
	AllGal[gal].CM_Pos[1]=AllGal[gal].Pos[1];
	AllGal[gal].CM_Pos[2]=AllGal[gal].Pos[2];
	#endif
//////////
//Now we have set uo center of mass for this galaxy
//loop on particles and calculate the force
    for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
	{//2nd for open
	#ifdef Mass_Adjust
                Adjust_Particlemass(gal,i);
	#endif
	force_gal(gal, i);
	}//2nd for close
    ...
    Match_ids(); //Find particles in the halos with galaxies.  Calculate the galaxy force on those particles.
  
    #endif

  /*  if(All.SnapshotFileCount > 160)
    {
      sprintf(filelen, "%s/data/mbid_data_%d.txt", All.OutputDir, ThisTask);
      data = fopen(filelen, "a+");

      for(aa=0;aa<NumPart;aa++)
      {
        for(jj=0;jj<NumGalaxies;jj++)
        {
           if(P[aa].ID == AllGal[jj].mostbound)
           {
               
               fprintf(data, "%f %d %u %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n", All.Time, All.SnapshotFileCount-1, AllGal[jj].mostbound, jj, AllGal[jj].Mvir, AllGal[jj].Pos[0], AllGal[jj].Pos[1], AllGal[jj].Pos[2], AllGal[jj].Vel[0], AllGal[jj].Vel[1], AllGal[jj].Vel[2], P[aa].Pos[0], P[aa].Pos[1], P[aa].Pos[2], P[aa].Vel[0], P[aa].Vel[1], P[aa].Vel[2], P[aa].g.GravAccel[0], P[aa].g.GravAccel[1], P[aa].g.GravAccel[2]);
           }
        }
      }
       fclose(data);
 ...
 */ 
     ...
     /*if(ThisTask == 0)
     {
        ...
        sprintf(filelen, "%s/data_step.txt", All.OutputDir);
        data = fopen(filelen, "a+");
        fprintf(data, "%d %f\n", All.SnapshotFileCount-1, All.Time);
        fclose(data);
     }*/
#ifdef Extrap
  /*if(ThisTask == 0)
  ...
  if(snap_oldgal_count == 1)
  {
    ...
    for(aa = 0; aa < OldNumGalaxies; aa++)
    {

      fprintf(data, "%d %f %f %f %f %f %f %f %f %f %f %f %f\n", All.SnapshotFileCount-1, All.Time, OldGal[aa].oldColdGas, OldGal[aa].oldStellarMass, OldGal[aa].oldBulgeMass, OldGal[aa].oldBlackHoleMass, OldGal[aa].oldDiskScaleRadiusAllGal[aa].NewJ[0], AllGal[aa].NewJ[1], AllGal[aa].NewJ[2], AllGal[aa].NewGalPos[0], AllGal[aa].oldGalPos[1], AllGal[aa].oldGalPos[2]);
    
    }
    fclose(data);
  }*/

  /*if(snap_oldgal_count >1 && All.SnapshotFileCount > 460)
  {
   
    sprintf(filelen, "%s/data_step.txt", All.OutputDir);
    data = fopen(filelen, "a+");

    for(aa = 0; aa < NumGalaxies; aa++)
    {

      fprintf(data, "%d %f %f %f %f %f %f %f %f %f %lld %f %f %f %f %f %f\n", All.SnapshotFileCount-1, All.Time, AllGal[aa].NewColdGas, AllGal[aa].NewStellarMass, AllGal[aa].NewBulgeMass, AllGal[aa].NewBlackHoleMass, AllGal[aa].NewDiskScaleRadius, AllGal[aa].NewJ[0], AllGal[aa].NewJ[1], AllGal[aa].NewJ[2], AllGal[aa].mostbound, AllGal[aa].NewGalPos[0], AllGal[aa].NewGalPos[1], AllGal[aa].NewGalPos[2], AllGal[aa].Pos[0], AllGal[aa].Pos[1], AllGal[aa].Pos[2]);
...
/*void *Galaxy_tree_force(void *p, int Firstactparticle, int Nextactparticle)
{
   int i;
   int b=0;
   for(i=0; i<NumGalaxies; i++)
   {
      for (b=FirstActiveParticle; b>=0; b=NextActiveParticle[b])
      {
          if(AllGal[i].mostbound == P[b].ID)
          {
              force_treewalk(i, b, 0);
          }
       }
    }
