...
OPTIMIZE = -O2 -Wall  -g   # optimization and warning flags (default)
...
# ===== COSANG detection =====
# 1) Prefer what config-makefile wrote into gadgetconfig.h (CONFIGVARS)
COSANG_IN_CONFIGVARS := $(and \
  $(findstring tree_sage_run,$(CONFIGVARS)), \
  $(findstring THIS_IS_A_COSANG_RUN,$(CONFIGVARS)) \
)

# ===== TAGGING COde detection =====
# 1) Prefer what config-makefile wrote into gadgetconfig.h (CONFIGVARS)
#TAG_IN_CONFIGVARS := $(or \
#  $(findstring TAG_IN_COLLECTIVE,$(CONFIGVARS)), \
#  $(findstring TAG_IN_HDF,$(CONFIGVARS)), \
#  $(findstring PARTICLE_TAGGING,$(CONFIGVARS))
#)


TAG_IN_CONFIGVARS := $(strip $(filter TAG_IN_COLLECTIVE TAG_IN_HDF PARTICLE_TAGGING,$(CONFIGVARS)))


# 2) Decide COSANG mode based on config-makefile output
ifeq ($(COSANG_IN_CONFIGVARS),)
  $(info Either COSANG and or tree_sage_run Not detected !!! COSANG_IN_CONFIGVARS = '$(COSANG_IN_CONFIGVARS)')
  COSANG_MODE := 0
else
  COSANG_MODE := 1
  $(info Confirmed COSANG_MODE = '$(COSANG_IN_CONFIGVARS)' from gadgetconfig.h)
endif

# 3) Decide TAGGING mode based on config-makefile output
ifeq ($(TAG_IN_CONFIGVARS),)
  $(info Particle Tagging Not detected !!! TAG_IN_CONFIGVARS = '$(TAG_IN_CONFIGVARS)')
  TAGGING_MODE := 0
else
  TAGGING_MODE := 1
  $(info Confirmed TAGGING_MODE = '$(TAG_IN_CONFIGVARS)' from gadgetconfig.h)
endif
...
$(info #                                                                      #)
$(info #                                                                      #)
$(info #       PROJECT: COSANG  —  Coupling Semi-Analytic and N-body Galaxies #)
$(info #                                                                      #)
...
#----------------------------------------------------------------------------------------------
ifeq ($(SYSTYPE),"uahpc")
CC       =  mpicc -g -O2 #-xW -ipo -Wall #-g means debugging information -02 means optimization level 2
CXX      =  mpiCC -g -O2 -xW -ipo -Wall
OPTIMIZE = -g -O2 -Wall
GMP_INCL = -I/home/bbhattarai2/include #Make changes as needed Download GMP from http://gmplib.org/
GMP_LIBS = -L/home/bbhattarai2/lib

GSL_INCL = -I/share/apps/gsl/gsl-2.7.1/include #Make changes as needed Download GSL from http://www.gnu.org/software/gsl/
GSL_LIBS = -L/share/apps/gsl/gsl-2.7.1/lib

FFTW_INCL= -I/home/bbhattarai2/include
FFTW_LIBS= -L/home/bbhattarai2/lib
#HDF5INCL =  -DH5Dcreate_vers=2#skiping this as module load hdf5/1.8.18_mpt takes care of this (Tested version 1 and 2 flags)

#HDF5INCL =  -I/share/apps/include
#HDF5LIB  =  -L/share/apps/lib -lhdf5 -lz
#HDF5INCL =  -DH5Dcreate_vers=2
HDF5LIB  =  -L/cm/shared/apps/hdf5/1.12.1/lib -lhdf5 -lz
#HDF5LIB  =  -L/cm/shared/apps/hdf5/1.12.1/lib

HDF5INCL =  -I/cm/shared/apps/hdf5/1.12.1/include

#HDF5INCL = -I$($HDF5INCLUDE)
#HDF5LIB  = -L$(HDF5LIB) -lhdf5 -lz
...
CC       =  mpicc -g -O0 #-xW -ipo -Wall #-g means debugging information -02 means optimization level 2
CXX      =  mpiCC -g -O0 -xW -ipo -Wall
OPTIMIZE = -g -O0 -Wall
GMP_INCL = -I/home6/bbhattar/include #Make changes as needed Download GMP from http://gmplib.org/
GMP_LIBS = -L/home6/bbhattar/lib
GSL_INCL = -I/home6/bbhattar/include #Make changes as needed Download GSL from http://www.gnu.org/software/gsl/
GSL_LIBS = -L/home6/bbhattar/lib
FFTW_INCL= -I/home6/bbhattar/include
FFTW_LIBS= -L/home6/bbhattar/lib
#HDF5INCL =  -DH5Dcreate_vers=2#skiping this as module load hdf5/1.8.18_mpt takes care of this (Tested version 1 and 2 flags)
HDF5INCL =  #skiping this as module load hdf5/1.8.18_mpt takes care of this
HDF5LIB  =  -lhdf5 -lz #
endif
...
ifeq ($(SYSTYPE),"TACC")
CC       = mpicc# /share/apps/intel/impi/4.0.1.007/intel64/bin/mpicc# /share/apps/intel/2019/impi/2019.5.281/intel64/bin/mpicc#/share/apps/intel/impi/4.0.1.007/intel64/bin/mpicc #mpicc  # /usr/local/x86_64/gnu/openmpi-1.4.5/bin/mpicc   # sets the C-compiler
CXX      =  mpiCC
ifeq (SOFTDOUBLEDOUBLE,$(findstring SOFTDOUBLEDOUBLE,$(CONFIGVARS)))
CC       =  mpiCC
OPT     +=  -DX86FIX
...
GSL_INCL =  -I/opt/apps/intel18/gsl/2.3/include    #-I/usr/local/x86_64/gnu/gsl-1.9/include      #-I/afs/mpa/home/volker/Libs/include
GSL_LIBS =  -L/opt/apps/intel18/gsl/2.3/lib -Wl,"-R /opt/apps/intel18/gsl/2.3/lib" #-L/usr/local/x86_64/gnu/gsl-1.9/lib   #-L/afs/mpa/home/volker/Libs/lib
FFTW_INCL=  -I/home1/07548/tg868478/source/fftw-2.1.5/include  #-I/lustre/projects/p004_swin/shared/my_fftw_install/include    #-I/afs/mpa/home/volker/Libs/include
FFTW_LIBS=  -L/home1/07548/tg868478/source/fftw-2.1.5/lib -Wl,"-R /home1/07548/tg868478/source/fftw-3.3.8/lib"  #-L/lustre/projects/p004_swin/shared/my_fftw_install/lib     #-L/afs/mpa/home/volker/Libs/lib -Xlinker -R -Xlinker /afs/mpa/home/volker/Libs/lib
MPICHLIB =
HDF5INCL =  -I/opt/apps/intel18/hdf5/1.8.16/x86_64/include  -Wall -DH5_USE_16_API  #-I/usr/local/x86_64/gnu/hdf5-1.8.9-openmpi-psm/include   #-I/afs/mpa/home/volker/Libs/include
HDF5LIB  =  -L/opt/apps/intel18/hdf5/1.8.16/x86_64/ -lhdf5_hl -lhdf5 -lz -lm  #-L/usr/local/x86_64/gnu/hdf5-1.8.9-openmpi-psm/lib -lhdf5 -lz  #-L/afs/mpa/home/volker/Libs/lib -lhdf5 -lz
...
MPICHLIB =
HDF5INCL =  -I/share/apps/include  -Wall -DH5_USE_16_API  #-I/usr/local/x86_64/gnu/hdf5-1.8.9-openmpi-psm/include   #-I/afs/mpa/home/volker/Libs/include
HDF5LIB  =  -L/share/apps/lib -lhdf5_hl -lhdf5 -lz -lm  #-L/usr/local/x86_64/gnu/hdf5-1.8.9-openmpi-psm/lib -lhdf5 -lz  #-L/afs/mpa/home/volker/Libs/lib -lhdf5 -lz
#IPM_INTEL = -L/share/apps/ipm/intel/lib -lipm  
OPT     +=  -DOLD_HDF5
OPT     +=  -DUNEQUALSOFTENINGS
endif
...
HDF5LIB  = -L$(TACC_HDF5_LIB) -lhdf5 -lz
MPICHLIB =
OPT     += -DUSE_MPI_IN_PLACE #I commented this
#OPT     += -MPI_IN_PLACE 
OPT     +=  -DOLD_HDF5
##
...
ifeq ($(SYSTYPE),"Anvil") #new Anvil Purdue University
CC       =  mpicc
CXX      =  mpic++
FC       =  mpif90 -nofor_main
#OPTIMIZE = -O3 $(TACC_VEC_FLAGS) -ipo -funroll-loops -no-prec-div -fp-model fast=2
...
OPT     +=  -DOLD_HDF5
##
## module load hdf5 gsl/2.4 fftw/2.1.5
endif
...
GRAVITY_OBJS  =  forcetree.o darkenergy.o accel.o   #pm_periodic.o accel.o
...
# EXEC   = P-Gadget3
ifeq ($(TAGGING_MODE),1)
EXEC   = P-Gadget3_cosang_tagged
else
EXEC   = P-Gadget3_cosang_untagged
endif
$(info #                                          #)
$(info #                                          #)
$(info #                                          #)
$(info #                                          #)
$(info Will save COSANG executable as = '$(EXEC)' )
$(info #                                          #)
$(info #                                          #)
$(info #                                          #)
$(info #                                          #)
...
EXEC   = non_CoSANG_Gadget3
$(info #                                          #)
$(info #                                          #)
$(info #                                          #)
$(info #                                          #)
$(info Will save Non COSANG executable as = '$(EXEC)' )
$(info #                                          #)
$(info #                                          #)
$(info #                                          #)
$(info #                                          #)
...
OBJS  = endrun.o mpi_util.o restart.o subfind_loctree.o additionalSPH.o fof.o  mymalloc.o  run.o  subfind_nearesttwo.o \
 ...
 cooling.o init.o pm_periodic.o subfind_collective.o system.o io.o potential.o subfind_cont.o timestep.o \
 density.o kicks.o predict.o subfind_density.o twopoint.o domain.o longrange.o read_ic.o subfind_distribute.o utilities.o \
 driftfac.o main.o read_ic_cluster.o subfind_findlinkngb.o sage.o core_init.o core_read_parameter_file.o core_io_tree.o \
 core_mymalloc.o core_build_model.o core_cool_func.o core_save.o model_cooling.o model_disk_instability.o model_infall.o \
 model_mergers.o model_misc.o model_reincorporation.o model_starformation_and_feedback.o model_z_dependent_sf.o \
 sage_pathsread.o  Halotree_read.o halo_pathnames.o sage_read.o lock_file.o \
 basetree.o system_bt.o mymalloc_bt.o treecreate.o mymalloc_ht.o peano_ht.o read_subp_single.o read_subp_multiple.o read_subfind_ids.o \
 sub_particle_list.o mpi_subfind_info.o create_plists.o Match_id_force.o hash.o disk_orient.o extrap.o center_of_mass.o timer.o \
 extrap_restart.o restart_cosang.o cosang_data_setup.o
...
INCL    += allvars.h      cooling.h      domain.h       forcetree.h    kernel.h       proto.h        swap.h         utilities.h \
	   assert.h       dd.h           fof.h           myqsort.h      subfind.h      tags.h         vector.h  \
           proto_bt.h hash.h core_proto.h core_simulation.h Makefile
...
LIBS   = -lm $(HDF5LIB) -g $(MPICHLIB) $(GSL_LIBS) -lgsl -lgslcblas $(FFTW)  # $(IPM_INTEL)
