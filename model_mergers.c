...
double estimate_merging_time(int sat_halo, int mother_halo, int ngal)
{
  ...
  coulomb = log(Halo_s[mother_halo].Len / ((double) Halo_s[sat_halo].Len) + 1);
  ...
  if(SatelliteMass > 0.0)
    mergtime = 2.0 *
    1.17 * SatelliteRadius * SatelliteRadius * get_virial_velocity(mother_halo) / (coulomb * All.G * SatelliteMass);
  else
    mergtime = -1.0;
...
void quasar_mode_wind(int gal, float BHaccrete)
{
  ...
  quasar_energy = QuasarModeEfficiency * 0.1 * BHaccrete * (C / All.UnitVelocity_in_cm_per_s) * (C / All.UnitVelocity_in_cm_per_s);
...
void collisional_starburst_recipe(double mass_ratio, int merger_centralgal, int centralgal, double time, double dt, int halonr, int mode)
{
  ...
  for(outputbin = 0; outputbin < NOUT; outputbin++)
  {
    if(Halo_s[halonr].SnapNum == ListOutputSnaps[outputbin])
    {
