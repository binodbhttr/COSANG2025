...
int whichfile(float *pos)
{
  ...
  key = peano_hilbert_key_ht(pos[0] * fac, pos[1] * fac, pos[2] * fac, BITS_PER_DIMENSION);

  sector = FilesPerSnapshot * ((double) key) / (1 << (3 * BITS_PER_DIMENSION));

  if(sector >= FilesPerSnapshot)
    sector = FilesPerSnapshot - 1;
