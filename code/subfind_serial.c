...
int subfind_process_group_serial(int gr, int Offs)
{
  ...
  double SubLum[LT_ADD_GAL_TO_SUB], SubLumAtt[LT_ADD_GAL_TO_SUB], SubLumObs[LT_ADD_GAL_TO_SUB], SubMeanAge,
    SubMeanZStar, SubSFR;
#ifdef DUSTATT
  double Dust[DUSTATT];
#else
...
{
//FILE *H_File;
//memset(&tagged_p,0,sizeof(struct tagged_particle));
//memset(&tagged_h,0,sizeof(struct tagged_halo));
//tagged_p.ID=10; // test
//char tag_path[100];
//char tag_halo_name[100]
//
sprintf(tag_path,"%s/tag_%03d",All.OutputDir,All.SnapshotFileCount-1);
//sprintf(tag_file_name,"%s/tagged_%03d/tagged_halos_serial_%03d_%03d",All.OutputDir,gr,gr,Nsubgroups);
int tag_file_status=mkdir(tag_path,02755);
//
}
#endif
  ...
  for(k = 0, subnr = 0, totlen = 0; k < nsubs; k++)
	...
	if(NumGalaxies>0)
	{
	memset(&tagged_p,-1,sizeof(struct tagged_particle));
//////memset(&tagged_h,0,sizeof(struct tagged_halo));
//All.SnapshotFileCount
//sprintf(tag_file_name,"%s/tagged_%03d/tagged_halos_serial_%03d_%03d_%03d",All.OutputDir,gr,gr,Nsubgroups,k);
//sprintf(tag_file_name,"%s/tagged_%03d/tagged_halos_serial_%03d_%03d_%03d",All.OutputDir,All.SnapshotFileCount-1,gr,Nsubgroups,k);
	#ifdef TAG_IN_HDF
//sprintf(tag_file_name,"%s/tag_%03d/tag_serial_%03d_%03d.h5",All.OutputDir,All.SnapshotFileCount-1,ThisTask,gr);
	sprintf(tag_file_name,"%s/tag_%03d/t_serial_%03d.h5",All.OutputDir,All.SnapshotFileCount-1,ThisTask);
	hid_t Tplist_id;
	//Tplist_id=H5Pcreate(H5P_FILE_ACCESS);
   	Tfile_id=H5Fcreate(tag_file_name, H5F_ACC_TRUNC, H5P_DEFAULT,H5P_DEFAULT);//Tplist_id);// H5P_DEFAULT);
	//H5Pclose(Tplist_id);
	#else
	sprintf(tag_file_name,"%s/tag_%03d/tag_serial_%03d_%03d",All.OutputDir,All.SnapshotFileCount-1,ThisTask,ThisTask);
	#endif
////int tag_file_status=mkdir(tag_path,02755);
	tagged_p.HaloIndex=subnr;
	tagged_p.Snap=All.SnapshotFileCount-1;
	tagged_p.Time=All.Time;
	tagged_p.SubhaloIndex=Nsubgroups;
	tagged_p.GalIndex=k;
	printf("SF_S390- subnr:%d,Time:%f, Gal:%d, p-Gal:%d\n",subnr,All.Time,k,tagged_p.GalIndex);
}
   #endif
	  ...
	  for(i = 0, minindex = -1, minpot = 1.0e30; i < len; i++)
	    {
	      ...
	      P[p].u.DM_Potential = pot + P[p].Mass * 2.8 / P[p].AGS_Hsml;
...
void load_CB07_table(int IMFt, int num)
{
  ...
  for(imet = 0; imet <= 6; imet++)
    {
      sprintf(buff, "%s/%s/cb2007_hr_stelib_m%d2_ssp.multi_mag_vega", All.BC_SED_Path, SFs[0].IMFname,
	      2 + imet);
      ...
      for(i = 0; i < 219; i++)
	{
	  ...
	  for(imag = 0; imag < LT_ADD_GAL_TO_SUB; ++imag)
	    {
	      fscanf(CB07File, "%f",
		     &CB07[imag + (i * LT_ADD_GAL_TO_SUB) + (imet * 219 * LT_ADD_GAL_TO_SUB)]);
	    }
	}
      fclose(CB07File);

#ifdef OBSERVER_FRAME
      sprintf(buff, "%s/%s/cb2007_hr_stelib_m%d2_ssp.multi_mag_vega_%03d", All.BC_SED_Path, SFs[0].IMFname,
	      2 + imet, num);
      if(!(CB07File = fopen(buff, "rb")))
	{
	  printf("Could not open file %s' !\n", buff);
...
#endif
{
  ...
  *SFRgas = 0;
  ...
  Unit_corrections_for_dust =
    All.UnitMass_in_g / All.HubbleParam / (1.4 * PROTONMASS * (All.UnitLength_in_cm / All.HubbleParam) *
					   (All.UnitLength_in_cm / All.HubbleParam) * 2.1 * 1.e21);
...
//Particle tagging test
#ifdef PARTICLE_TAGGING
//#ifdef TAG_IN_COLLECTIVE
if(NumGalaxies>0 && num>0)
{
double *bnd_energy2,bndE_limit;
struct tagged_particle *tagged_list;
int tagCount,tag_limit;
f_mb=10.0;
//tag_limit=((float)f_mb)*ndm;
tag_limit=(int)ndm/f_mb;
tagCount=0;
//if(ThisTask==0)
//printf("SF_S1117-num:%d, ndm:%d,tag_limit:%d, Time:%f\n",num,ndm,tag_limit,All.Time);
bnd_energy2 = (double *) mymalloc("bnd_energy2", num * sizeof(double));
//bnd_energy2=(double*)malloc(num*sizeof(double));
memset(bnd_energy2,1.0e20,sizeof(double));
for(i = 0, minindex = -1, minpot = 1.0e30; i < num; i++)
	{
		p = d[i].index;
		bnd_energy2[i] = P[p].v.DM_BindingEnergy;
	}

#ifdef OMP_SORT
      omp_qsort(bnd_energy2, num, sizeof(double), subfind_compare_binding_energy);       /* largest comes first! */
#else
      qsort(bnd_energy2, num, sizeof(double), subfind_compare_binding_energy);   /* largest comes first! */
#endif


bndE_limit=bnd_energy2[(int)(tag_limit)];
tagged_list=(struct tagged_particle *)mymalloc("tagged_list",tag_limit*sizeof(struct tagged_particle));
//tagged_list=malloc(tag_limit*sizeof(struct tagged_particle));
        for(i = 0,tagCount=0; i < num; i++)
        {
          p = d[i].index;
                if(P[p].v.DM_BindingEnergy<bndE_limit && tagCount<=tag_limit)
                {
			for(j=0;j<3;j++){
                                tagged_list[tagCount].Pos[j]=P[p].Pos[j];
                                tagged_list[tagCount].Vel[j]=P[p].Vel[j];
			}
                        tagged_list[tagCount].PID=P[p].ID;//what is absolute id?
                        tagged_list[tagCount].Snap=All.SnapshotFileCount-1;//tagged_p.Snap;
                        //tagged_list[tagCount].BindingEnergy=P[p].v.DM_BindingEnergy;
                        tagged_list[tagCount].HaloIndex=tagged_p.HaloIndex;
                        tagged_list[tagCount].SubhaloIndex=tagged_p.SubhaloIndex;
			tagged_list[tagCount].GalIndex=tagged_p.GalIndex;
			tagged_list[tagCount].GalNo=0;//tagged_p.GalIndex;
			tagged_list[tagCount].TreeIndex=0;//tagged_p.GalIndex;
                        tagged_list[tagCount].ID=p;
			tagged_list[tagCount].Time=All.Time;//tagged_p.Time;
			tagged_list[tagCount].BindingEnergy=P[p].v.DM_BindingEnergy;
			tagged_list[tagCount].AA=0;
                        tagged_list[tagCount].Age=0;
                        tagged_list[tagCount].ZZ=0;
			tagged_list[tagCount].StellarMass=0;
                        tagged_list[tagCount].Len=num;
                        tagged_list[tagCount].MBID=P[minindex].ID;
			tagged_list[tagCount].Mvir=0;
			tagged_list[tagCount].Rvir=0;
			tagged_list[tagCount].infallMvir=0;
			tagged_list[tagCount].LastMajorMerger=0;
			//printf("SF_S1153-ID:%d,PID:%ld,snap:%d,pos:%g,%g,%g, vel:%g,%g,%g\n",p,P[p].ID,tagged_p.Snap,P[p].Pos[0],P[p].Pos[1],P[p].Pos[2],P[p].Vel[0],P[p].Vel[1],P[p].Vel[2]);
                        tagCount++;
                }
        }
	//printf("SF_S1173- pSnap:%d, AllSnapC-1:%d,limit:%d,count:%d,firstID:%lld\n",tagged_p.Snap,All.SnapshotFileCount-1,tag_limit,tagCount,tagged_list[0].ID);
	#ifdef TAG_IN_HDF
	//int totCount;
	hid_t  Tdset_id, Tdatatype;//, Tgroup;
	hid_t Tfilespace;//, Tmemspace;
	hsize_t Tdimsf[2]; //1D array of struct, total size
	//hsize_t Tcount[2]; //1D array, local list
	//hsize_t Toffset[2]; // offset of the each slab
	//hid_t Tplist_id; // property list identifier
	herr_t Tstatus;
	//printf("SF_S1183\n");
	...
	//Tplist_id=H5Pcreate(H5P_FILE_ACCESS);
	//printf("SF_S1187\n");
	//H5Pset_fapl_mpio(Tplist_id, MPI_COMM_WORLD,MPI_STATUS_IGNORE);//Tinfo);
	//printf("SF_S1189\n");	
	//printf("SF_S1177- tag_file:%s\n",tag_file_name);
	//Tfile_id=H5Fcreate(tag_file_name, H5F_ACC_TRUNC, H5P_DEFAULT, Tplist_id);
	//H5Pclose(Tplist_id);
	//MPI_Allreduce(&tag_limit,&totCount, 1, MPI_INT,MPI_SUM,MPI_COMM_WORLD);
	//totCount=3*NTask;
	Tdimsf[0]=tag_limit;
	Tdimsf[1]=1;
	//printf("SF_S1192\n");	
	//tagfile=H5Fcreate("test.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
	//int tagdims[2],tagfile_status;
	typedef struct tagged_particle tagged_tmp;
	//tagdims[1]=19;//15;
	Tfilespace=H5Screate_simple(2,Tdimsf,NULL);
	//printf("SF_S1198\n");	
	Tdatatype=H5Tcreate(H5T_COMPOUND,sizeof(struct tagged_particle));//124);// sizeof(struct tagged_p));//H5Tcopy(H5T_NATIVE_INT);
	H5Tinsert(Tdatatype, "X", HOFFSET(tagged_tmp, Pos[0]), H5T_NATIVE_DOUBLE);
        H5Tinsert(Tdatatype, "Y", HOFFSET(tagged_tmp, Pos[1]), H5T_NATIVE_DOUBLE);
        H5Tinsert(Tdatatype, "Z", HOFFSET(tagged_tmp, Pos[2]), H5T_NATIVE_DOUBLE);
        H5Tinsert(Tdatatype, "Vx", HOFFSET(tagged_tmp, Vel[0]), H5T_NATIVE_DOUBLE);
        H5Tinsert(Tdatatype, "Vy", HOFFSET(tagged_tmp, Vel[1]), H5T_NATIVE_DOUBLE);
        H5Tinsert(Tdatatype, "Vz", HOFFSET(tagged_tmp, Vel[2]), H5T_NATIVE_DOUBLE);
	H5Tinsert(Tdatatype, "ID", HOFFSET(tagged_tmp, ID), H5T_NATIVE_LLONG);
	H5Tinsert(Tdatatype, "Snap", HOFFSET(tagged_tmp, Snap), H5T_NATIVE_INT);
	H5Tinsert(Tdatatype, "PID", HOFFSET(tagged_tmp, PID), H5T_NATIVE_LLONG);
	H5Tinsert(Tdatatype, "HaloIndex", HOFFSET(tagged_tmp, HaloIndex), H5T_NATIVE_INT);
	H5Tinsert(Tdatatype, "SubhaloIndex", HOFFSET(tagged_tmp, SubhaloIndex), H5T_NATIVE_INT);
	H5Tinsert(Tdatatype, "GalIndex", HOFFSET(tagged_tmp, GalIndex), H5T_NATIVE_INT);
        H5Tinsert(Tdatatype, "GalNo", HOFFSET(tagged_tmp, GalNo), H5T_NATIVE_INT);
        H5Tinsert(Tdatatype, "TreeIndex", HOFFSET(tagged_tmp, TreeIndex), H5T_NATIVE_INT);
	H5Tinsert(Tdatatype, "AA", HOFFSET(tagged_tmp, AA), H5T_NATIVE_FLOAT);
	H5Tinsert(Tdatatype, "Age", HOFFSET(tagged_tmp, Age), H5T_NATIVE_FLOAT);
	H5Tinsert(Tdatatype, "ZZ", HOFFSET(tagged_tmp, ZZ), H5T_NATIVE_FLOAT);
	H5Tinsert(Tdatatype, "StellarMass", HOFFSET(tagged_tmp, StellarMass), H5T_NATIVE_DOUBLE);
	H5Tinsert(Tdatatype, "Time", HOFFSET(tagged_tmp, Time), H5T_NATIVE_DOUBLE);
	H5Tinsert(Tdatatype, "Len", HOFFSET(tagged_tmp, Len), H5T_NATIVE_INT);
	H5Tinsert(Tdatatype, "MBID", HOFFSET(tagged_tmp, MBID), H5T_NATIVE_LLONG);
	H5Tinsert(Tdatatype, "BindingEnergy", HOFFSET(tagged_tmp, BindingEnergy), H5T_NATIVE_DOUBLE);
	H5Tinsert(Tdatatype, "Mvir", HOFFSET(tagged_tmp, Mvir), H5T_NATIVE_DOUBLE);
	H5Tinsert(Tdatatype, "Rvir", HOFFSET(tagged_tmp, Rvir), H5T_NATIVE_DOUBLE);
	H5Tinsert(Tdatatype, "infallMvir", HOFFSET(tagged_tmp, infallMvir), H5T_NATIVE_DOUBLE);
        H5Tinsert(Tdatatype, "LastMajorMerger", HOFFSET(tagged_tmp, LastMajorMerger), H5T_NATIVE_FLOAT);
	//H5Tinsert(tagdatatype, "ID", HOFFSET(tagged_tmp, ID), H5T_NATIVE_INT);
	//int cparms = H5Pcreate (H5P_DATASET_CREATE);	
	char Tdataset_name[150];//, group_name[100];
	//sprintf(group_name,"/tagHalo_%d_Subhalo_%d",tagged_p.HaloIndex,tagged_p.SubhaloIndex);
	//sprintf(group_name,"/MiniTag",tagged_p.HaloIndex,tagged_p.SubhaloIndex);
	//sprintf(Tdataset_name,"tagHalo_%d_Subhalo_%d/gr%d",tagged_p.HaloIndex,tagged_p.SubhaloIndex,group_name);
	//sprintf(Tdataset_name,"%s/gr%d", group_name, tagged_p.GalIndex);
	sprintf(Tdataset_name,"MiniTag");//, group_name);
	...
	//Tcount[0]=tag_limit;
	//Tcount[1]=Tdimsf[1];
	//Toffset[0]=ThisTask*Tcount[0];
	//Toffset[1]=0;
	//Tmemspace=H5Screate_simple(1, Tcount,NULL);
	//Tfilespace=H5Dget_space(Tdset_id);
	//printf("SF_S1232\n");
	//H5Sselect_hyperslab(Tfilespace, H5S_SELECT_SET,Toffset,NULL, Tcount,NULL);
	//printf("SF_S1232\n");
	//Tplist_id=H5Pcreate(H5P_DATASET_XFER);
	//H5Pset_dxpl_mpio(Tplist_id, H5FD_MPIO_COLLECTIVE);
	//printf("SF_S1235\n");
	//tagdataset=H5Dcreate("test.h5",tagdataset_name,tagdatatype, tagdataspace, H5P_DEFAULT);
	//tagfile_status=H5Dwrite(tagdataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data"?");
	Tstatus=H5Dwrite(Tdset_id, Tdatatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, tagged_list);
	//printf("SF_S1206- file:%d\n",tagfile);
	//printf("SF_S1207- dataspace:%d\n",tagdataspace);
	//printf("SF_S1208- size,tmp:%d,list:%d\n",sizeof(tagged_tmp),sizeof(struct tagged_particle));
	//H5Gclose(Tgroup);
	H5Dclose(Tdset_id);
	H5Tclose(Tdatatype);
	H5Sclose(Tfilespace);
...
#else
FILE *P_File;
 if(!(P_File=fopen(tag_file_name,"ab")))
   {
...
if (ThisTask==0)
my_fwrite(tagged_list,sizeof(tagged_p),tag_limit,P_File);
...
#ifdef LT_ADD_GAL_TO_SUB
  rr_list_gal = (sort_r2list *) mymalloc("rr_list_gal", sizeof(sort_r2list) * num);
#endif
  if(num_use > 0)
...
/*--- Note that here we might want to correct for the baryon fraction ?? ---*/
  ...
  for(k = 0; k < DUSTATT - 1; k++)
      ...
      if(Mcold_in_bin[k] > 0)
	{
	  ...
	  TauV_in_bin[k] =
	    pow(Zgas_in_bin[k],
		1.6) * (Mcold_in_bin[k] / ((2 * k + 1) * 3.141516 *
					   pow(r_lum / (DUSTATT - 2) * All.Time,
					       2))) * Unit_corrections_for_dust;
	}
  ...
  for(i = 0, istar = 0; i < num; i++)
      ...
      if(P[rr_list_gal[i].index].Type == 4 && r_lum > 0)
	{
	  if(rr_list_gal[i].r <= r_lum)
		...
		{
		  if(k >= rad_bin_z)
		    term_depth1 += TauV_in_bin[k];
		  if(k <= rad_bin_z)
		    term_depth2 += TauV_in_bin[k];
		  term_depth_total += TauV_in_bin[k];
		}
	      if(term_depth_total > 0.)
		{
		  if(rr_list_gal[i].z >= 0)
		    depth = term_depth1 / (2 * term_depth_total);
		  if(rr_list_gal[i].z < 0)
		    depth = (term_depth2 + term_depth_total) / (2 * term_depth_total);
		}
	      else
		depth = 0.;
	      ...
	      for(ilum = 0; ilum < LT_ADD_GAL_TO_SUB; ilum++)
		{
		  ...
		  lum_att[ilum] +=
		    lum_particle[ilum] * exp(-depth * Tau_Lambda_over_V * TauV_in_bin[rad_bin]);
#ifdef OBSERVER_FRAME
		  Tau_Lambda_over_V_obs = mu * pow(Filters_Effective_Wavelenght[ilum] * All.Time / 550, -0.7);
		  lum_obs[ilum] +=
		    lum_particle_obs[ilum] * exp(-depth * Tau_Lambda_over_V_obs * TauV_in_bin[rad_bin]);
#endif
		}
#endif
...
#ifdef LT_ADD_GAL_TO_SUB
  if(nstarloc > 0)
    loc_rr_list_star = (sort_r2list *) mymalloc("loc_rr_list_star", sizeof(sort_r2list) * nstarloc);
  i_star = 0;
#endif
	  ...
	  if(P[part_index].Type == 4 && r_lum > 0)
	      ...
	      if(rr_tmp <= r_lum)
		{
		  ...
		  for(ilum = 0; ilum < LT_ADD_GAL_TO_SUB; ilum++)
			...
			lum_particle[ilum] * exp(-depth * Tau_Lambda_over_V * TauV_in_bin[rad_bin]);
#ifdef OBSERVER_FRAME
		      Tau_Lambda_over_V_obs =
			mu * pow(Filters_Effective_Wavelenght[ilum] * All.Time / 550, -0.7);
		      loc_lum_obs[ilum] +=
			lum_particle_obs[ilum] * exp(-depth * Tau_Lambda_over_V_obs * TauV_in_bin[rad_bin]);
#endif
		    }
...
#ifdef PARTICLE_TAGGING 
printf("Collective particle tagging!\n");
/////////New algorithm
...
int tag_file_status=mkdir(tag_path,02755);
#ifdef TAG_IN_HDF
sprintf(tag_file_name,"%s/tag_%03d/t_collective_%03d.h5",All.OutputDir,All.SnapshotFileCount-1,ThisTask);
//hid_t Tplist_id;
//Tplist_id=H5Pcreate(H5P_FILE_ACCESS);
Tfile_id=H5Fcreate(tag_file_name, H5F_ACC_TRUNC, H5P_DEFAULT,H5P_DEFAULT);//Tplist_id);// H5P_DEFAULT);
//H5Pclose(Tplist_id);
#else
sprintf(tag_file_name,"%s/tag_%03d/tag_collective_%03d_%03d",All.OutputDir,All.SnapshotFileCount-1,ThisTask,ThisTask);
#endif

double *bnd_energy2,bndE_limit=0;
struct tagged_particle *tagged_list;
int tagCount,tag_limit,ndmGlobal;
long int p;
f_mb=10.0;
//tag_limit=((float)f_mb)*ndm;
tag_limit=(int)ndm/f_mb;
...
parallel_sort(bnd_energy2, num, sizeof(double), subfind_compare_binding_energy);
double minBEGlobal,minBEloc=0;//minBEloc is the local min, minBE2 is the 2nd min
//double del,Del;
//int minBEcpu;
long int locCount=0,totCount=0;
//float ratio=1;
//minBEloc2=1e30;
minBEloc=bnd_energy2[0];
/* old collective version
MPI_Allreduce(&minBEloc,&minBEGlobal,1,MPI_DOUBLE,MPI_MIN,MPI_COMM_WORLD);
if(minBEGlobal == minBEloc)
	{
	maxBE0=bnd_energy2[num-1];
	del=maxBE0-minBEGlobal;
	del/=10;
	minBEcpu=ThisTask;
	}
MPI_Bcast(&del,1,MPI_DOUBLE,minBEcpu,MPI_COMM_WORLD);

bndE_limit=minBE+del;
//while(ratio>1e-6 && ratio<-1e-6)
do
{
bndE_limit+=del*ratio;
locCount=0;
for(i=0;i<num;i++)
{
	p = d[i].index;
	if(P[p].v.DM_BindingEnergy<bndE_limit)
		locCount++;
}
MPI_Allreduce(&locCount,&totCount,1,MPI_LONG_INT,MPI_SUM,MPI_COMM_WORLD);
ratio=(tag_limit-totCount)/tag_limit;

}while(abs(ratio)>1e-4 );
// old collective version */
//
//bndE_limit=bnd_energy2[(int)(tag_limit)];
//New collective tagging
f_mb=10.0;
//tag_limit=((float)f_mb)*ndm;
MPI_Allreduce(&num,&ndmGlobal,1,MPI_INT,MPI_SUM,MPI_COMM_WORLD);
tag_limit=(int)ndmGlobal/f_mb;

MPI_Allreduce(&minBEloc,&minBEGlobal,1,MPI_INT,MPI_MIN,MPI_COMM_WORLD);
bndE_limit=minBEGlobal;
while(totCount<=tag_limit);
{
locCount=0;
bndE_limit+=(bndE_limit/100);
for(i=0;i<num;i++)
	{
        p = d[i].index;
        if(P[p].v.DM_BindingEnergy<bndE_limit)
...
//tagCount=0;
//end of new section
tagged_list=(struct tagged_particle *)mymalloc("tagged_list",locCount*sizeof(struct tagged_particle));
//tagged_list=malloc(tag_limit*sizeof(struct tagged_particle));
        for(i = 0,tagCount=0; i < num; i++)
        {
          p = d[i].index;
        ...
        //sprintf(tagdataset_name,"%s",tag_path);
        //dset_id = H5Dcreate(file_id, DATASETNAME, H5T_NATIVE_INT, filespace,H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        Tdset_id=H5Dcreate(Tfile_id,Tdataset_name,Tdatatype, Tfilespace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);// H5P_DEFAULT, H5P_DEFAULT); //remove two H5P_DEFAULT for older version of hdf5
        //printf("SF_S1225\n");
        //H5Sclose(Tfilespace);
        //Tcount[0]=tag_limit;
        //Tcount[1]=Tdimsf[1];
...
//////////////////////////
//        int *min_bnd_ID;
//        int dm_count=0,task_index;
        
//        int tag_max;
//        struct min_bnd{
//        double BE;
//        int Rank;
//        }*minBE,*min_bnd_energy;
//        minindex=-1;
 //      
 //       min_bnd_ID=(int*)mymalloc("min_bnd_ID",NTask*sizeof(int));
 //       min_bnd_energy=(struct min_bnd*)mymalloc("min_bnd_energy",sizeof(struct min_bnd));
 //       for(i=0;i<num;i++)
 //       {
 //               part_index=d[i].index;
  //              if(P[part_index].v.DM_BindingEnergy<(*min_bnd_energy).BE || minindex==-1 )
    //            {
      //                  (*min_bnd_energy).BE=P[part_index].v.DM_BindingEnergy;
        //                min_bnd_ID[ThisTask]=part_index;
        //                (*min_bnd_energy).Rank=ThisTask;
        //        }

    //    }
        
//        MPI_Allreduce(min_bnd_energy,minBE,1,MPI_DOUBLE_INT,MPI_MINLOC,MPI_COMM_WORLD);
//        tag_max=f_mb*ndm;
 //       if(ThisTask==0)
 //       {
 //               tagged_p.PID=min_bnd_ID[(*minBE).Rank];//minindex;
 //               ++tagged_p.ID;
 //               tagged_p.Time=All.Time;
 //               tagged_p.Snap=num;
 //               tagged_p.BindingEnergy=(*minBE).BE;

//		FILE *P_File;
//		 if(!(P_File=fopen(tag_file_name,"ab")))
//   		{
//        		printf("Can't open tagged particles file !\n");
//        		endrun(90001);
//   		}
//		my_fwrite(&tagged_p,sizeof(tagged_p),1,P_File);
