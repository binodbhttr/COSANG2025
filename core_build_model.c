...
void construct_galaxies(int halonr)
{
  ...
  HaloAux_s[halonr].DoneFlag = 1;
  halosdone++;

  prog = Halo_s[halonr].FirstProgenitor;
  while(prog >= 0)
  {
    if(HaloAux_s[prog].DoneFlag == 0)
      construct_galaxies(prog);
    prog = Halo_s[prog].NextProgenitor;
  }

  fofhalo = Halo_s[halonr].FirstHaloInFOFgroup;
  if(HaloAux_s[fofhalo].HaloFlag == 0)
  {
    HaloAux_s[fofhalo].HaloFlag = 1;
    while(fofhalo >= 0)
    {
      prog = Halo_s[fofhalo].FirstProgenitor;
      while(prog >= 0)
      {
        if(HaloAux_s[prog].DoneFlag == 0)
          construct_galaxies(prog);
      ...
      fofhalo = Halo_s[fofhalo].NextHaloInFOFgroup;
    }
  }
  ...
  fofhalo = Halo_s[halonr].FirstHaloInFOFgroup;
  if(HaloAux_s[fofhalo].HaloFlag == 1)
  {
    ngal = 0;
    HaloAux_s[fofhalo].HaloFlag = 2;
    ...
    evolve_galaxies(Halo_s[halonr].FirstHaloInFOFgroup, ngal);
  }
...
int join_galaxies_of_progenitors(int halonr, int ngalstart)
{
  ...
  lenmax = 0;
  lenoccmax = 0;
  first_occupied = Halo_s[halonr].FirstProgenitor;
  prog = Halo_s[halonr].FirstProgenitor;

  if(prog >=0)
    if(HaloAux_s[prog].NGalaxies > 0)
    lenoccmax = -1;
  ...
  while(prog >= 0)
  {
    if(Halo_s[prog].Len > lenmax)
    {
      lenmax = Halo_s[prog].Len;
      mother_halo = prog;
    }
    if(lenoccmax != -1 && Halo_s[prog].Len > lenoccmax && HaloAux_s[prog].NGalaxies > 0)
    {
      lenoccmax = Halo_s[prog].Len;
      first_occupied = prog;
    }
    prog = Halo_s[prog].NextProgenitor;
  }
  ...
  while(prog >= 0)
  {
    for(i = 0; i < HaloAux_s[prog].NGalaxies; i++)
    {
      if(ngal >= FoF_MaxGals)
      {
      ...
      Gal[ngal] = HaloGal[HaloAux_s[prog].FirstGalaxy + i];
      ...
      if(Gal[ngal].Type == 0 || Gal[ngal].Type == 1)
      {
        if(prog == first_occupied)
        {
          ...
          Gal[ngal].MostBoundID = Halo_s[halonr].MostBoundID;

          for(j = 0; j < 3; j++)
          {
            Gal[ngal].Pos[j] = Halo_s[halonr].Pos[j];
            Gal[ngal].Vel[j] = Halo_s[halonr].Vel[j];
          }

          Gal[ngal].Len = Halo_s[halonr].Len;
          Gal[ngal].Vmax = Halo_s[halonr].Vmax;
          ...
          if(halonr == Halo_s[halonr].FirstHaloInFOFgroup)
          {
            // central galaxy
          ...
          else
          {
            ...
            if(Gal[ngal].Type == 0 || Gal[ngal].MergTime > 999.0)
              ...
              Gal[ngal].MergTime = estimate_merging_time(halonr, Halo_s[halonr].FirstHaloInFOFgroup, ngal);
            Gal[ngal].Type = 1;
          }
        }
        else
...
void evolve_galaxies(int halonr, int ngal)	// note: halonr is here the FOF-background subhalo (i.e. main halo) 
{
  ...
  /* ============================================================================
     ...
     not classified as a central galaxy (Type != 0), or its parent subhalo number 
     (HaloNr) does not match the main subhalo number of the FOF group (halonr).
     
     The prints below dump the exact index, count, and structure contents to 
     help diagnose why the mapping mismatch occurred.
     ============================================================================ */
  centralgal = Gal[0].CentralGal;
  if(centralgal < 0 || centralgal >= ngal || Gal[centralgal].Type != 0 || Gal[centralgal].HaloNr != halonr)
  {
    printf("Task %d: evolve_galaxies error - centralgal=%d, ngal=%d, halonr=%d\n", ThisTask, centralgal, ngal, halonr);
    if (centralgal >= 0 && centralgal < ngal) {
      printf("  Gal[centralgal].Type=%d, Gal[centralgal].HaloNr=%d\n", Gal[centralgal].Type, Gal[centralgal].HaloNr);
    }
    printf("  Gal[0].CentralGal=%d, Gal[0].HaloNr=%d, Gal[0].Type=%d\n", Gal[0].CentralGal, Gal[0].HaloNr, Gal[0].Type);
    fflush(stdout);
    ABORT(54);
  }
  /*
  printf("Debugging info for evolve_galaxies:\n");
  printf("DBG: halonr=%d Snap=%d ngal=%d\n", halonr, Halo_s[halonr].SnapNum, ngal);
  printf("DBG: centralgal=%d Type=%d HaloNr=%d (expected %d)\n", centralgal, Gal[centralgal].Type, Gal[centralgal].HaloNr, halonr);
  fflush(stdout);
  */
  ...
  infallingGas = infall_recipe(centralgal, ngal, ZZ[Halo_s[halonr].SnapNum]);
  ...
  for(nstep = 0; nstep < STEPS; nstep++)
  {
    ...
    for(p = 0; p < ngal; p++)
    {
      ...
      deltaT = Age[Gal[p].SnapNum] - Age[Halo_s[halonr].SnapNum];
      time = Age[Gal[p].SnapNum] - (nstep + 0.5) * (deltaT / STEPS);
      if(ThisTask == 0)
      {
        // printf("deltaT = %f, time = %f\n", deltaT, time);
      }
  ...
  deltaT = Age[Gal[0].SnapNum] - Age[Halo_s[halonr].SnapNum];
  for(p = 0; p < ngal; p++)
  {
  ...
  for(p = 0, currenthalo = -1; p < ngal; p++)
  {
    if(Gal[p].HaloNr != currenthalo)
    {
      ...
      HaloAux_s[currenthalo].FirstGalaxy = NumGals;
      HaloAux_s[currenthalo].NGalaxies = 0;
    }
    ...
    if(Gal[p].Type != 3)
    {
      ...
      Gal[p].SnapNum = Halo_s[currenthalo].SnapNum;
      // Gal[p].GalaxyNr = NumGals;
      HaloGal[NumGals++] = Gal[p];
      HaloAux_s[currenthalo].NGalaxies++;
