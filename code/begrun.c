...
#pragma omp master
...
#if defined(PM_FROM_L3) && defined(PMGRID)
  create_plans();
#endif
  ...
  if(RestartFlag == 0 || RestartFlag == 2 || RestartFlag == 3 || RestartFlag == 4 || RestartFlag == 5
	...
	sqrt(All.UnitTime_in_s * All.UnitTime_in_s * All.UnitLength_in_cm / All.UnitMass_in_g);
      All.Alfa0Quench = all.Alfa0Quench;
      All.AlfaB2eq = gauss2gadget * gauss2gadget * all.AlfaB2eq;
#endif
#ifdef BLACK_HOLES
...
#endif
      printf("Timemax old = %g, Timemax new=%g\n", All.TimeMax, all.TimeMax);
  ...
  All.AGS_MaxSoft = AGS_MAXSOFT * All.Asmth[0];
  All.AGS_MinSoft = AGS_MINSOFT * All.Asmth[0];
  if(ThisTask == 0)
    printf("*ADAPTGRAVSOFT* MinSoft, MaxSoft= %g %g\n", All.AGS_MinSoft, All.AGS_MaxSoft);
#endif
...
void open_outputfiles(void)
{
  ...
  if(ThisTask == 0)
    {
      sprintf(buf, "%s/promotionfiles/", All.OutputDir);
      mkdir(buf, 02755);
    }
  MPI_Barrier(MPI_COMM_WORLD);

  sprintf(buf, "%s/promotionfiles/promotion_%d.txt", All.OutputDir, ThisTask);
  ...
  sprintf(buf, "%s%s", All.OutputDir, "multigrid_times.txt");
  if(!(FdMultiGridTimes = fopen(buf, mode)))
    {
      printf("error in opening file '%s'\n", buf);
      endrun(1);
    }
...
#define MAXTAGS 300
  ...
  if(ThisTask == 0)		/* read parameter file on process 0 */
	...
	for(x = 0; x < NUMCRPOP - 1; x++)
      ...
      strcpy(tag[nt], "MaxLevelToptree");
      addr[nt] = &All.MaxLevelToptree;
      id[nt++] = INT;
...
#ifdef FS_ALFA2_QUENCH
      strcpy(tag[nt], "Alfa0Quench");
      addr[nt] = &All.Alfa0Quench;
      id[nt++] = REAL;

      strcpy(tag[nt], "AlfaB2eq");
      addr[nt] = &All.AlfaB2eq;
      id[nt++] = REAL;
#endif

#ifdef FS_DYN_DEF
      strcpy(tag[nt], "Eta2Min");
      addr[nt] = &All.Eta2Min;
      id[nt++] = REAL;

      strcpy(tag[nt], "Eta2Max");
      addr[nt] = &All.Eta2Max;
      id[nt++] = REAL;

      strcpy(tag[nt], "Alfa2H");
      addr[nt] = &All.Alfa2H;
      id[nt++] = REAL;
#endif
      ...
      strcpy(tag[nt], "ZDiffusionCoeff");
      addr[nt] = &All.ConstantD;
      id[nt++] = REAL;
#endif

      strcpy(tag[nt], "MaxDiffusionTimeStep");
      addr[nt] = &All.MaxDiffusionTimeStep;
      id[nt++] = REAL;

      strcpy(tag[nt], "DiffusionCourantFac");
      addr[nt] = &All.DiffusionCourantFac;
      id[nt++] = REAL;

      strcpy(tag[nt], "DiffusionCourantMinFac");
      addr[nt] = &All.DiffusionCourantMinFac;
      id[nt++] = REAL;

      strcpy(tag[nt], "UseStdCourantDiffusion");
      addr[nt] = &All.StandardCourantDiffusion;
      id[nt++] = INT;
#endif
  ...
  if(ThisTask == 0)
    printf("UNEQUALSOFTENINGS has to be switched of if ADAPTGRAVSOFT is used \n");
  endrun(0);
#endif
#endif
