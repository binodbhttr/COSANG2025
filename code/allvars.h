...
#ifdef AB_SHOCK
#define SHOCK_GRADIENT_TOLERANCE 1.2
#define SHOCK_MINIMAL_VDIF_FRACTION 0.0001
#define SHOCK_NOISELEVEL 0.05
#endif
...
extern int OldNumGalaxies;
extern int Particle_List_tracker;
extern int snap_oldgal_count;
extern int galsearchcount;
extern int subtotids;
extern int NumSfiles;
extern int pidcounter;
extern unsigned int *particle_list;
extern int *Num_Part_list;
extern int NumTreefiles;
extern int *Numids;
extern int *sumNumids;
extern int *NumGroups;
extern int *SumNumgrps;
extern int InitNumPart;
extern int counter3;
extern unsigned int *P_list;
extern int *O_list;
extern int *G_list;
extern unsigned int *Temp_pid_list;
extern unsigned int *Temp_pid_list1;
extern unsigned int *Temp_pid_list2;
extern float CM_Px;
extern float CM_Py;
extern float CM_Pz;
extern float CM_M;
extern float *CM_Pxlist;
extern float *CM_Pylist;
extern float *CM_Pzlist;
extern float *CM_Mlist;
...
extern size_t HighMark_cs_enrich;
#endif
extern size_t HighMark_cs_hotngbs;
#endif
...
#ifdef MODGRAV
extern int NumSavedAMRCells; /*!< number of AMR cells from old tree saved on LOCAL task */
extern int NumAMRCells; /*!< number of AMR cells on LOCAL task */

extern int NumSavedMultigridCells;  /*!< number of multigird cells from old tree saved on LOCAL task */
extern int SavedMultigridMinLevel;  /*!< minimum tree level for which multigrid cells have been saved */
extern int SavedMultigridMaxLevel;  /*!< maximum tree level for which multigrid cells have been saved */

extern int FirstAMRCell; /*!< index of first AMR node */
extern int *NextAMRCell, *NextAMRCell_base; /*!< defined for all nodes, for AMR nodes it gives the index of the next AMR node */

extern int phi_guess_available;

extern struct amr_cell_data /*!< used to store AMR data of old tree */
{
  MyFloat center[3];
  int level;
  int bitflags;
  ...
  int target_task;
} *saved_amr_cells, *saved_multigrid_cells;

extern double *mg_time_multi_grid_nbg_search;
extern double *mg_time_multi_grid_update_cells;
extern double *mg_time_multi_grid_update_cells_comm_imbal;
extern int *mg_num_it_multi_grid;
extern long long *mg_num_cells_multi_grid;
#endif
...
extern FILE *FdMultiGridTimes;  /*!< file handle for multi_grid_times.txt log-file */
#endif
...
extern struct global_data_all_processes
{
...
#ifdef MODGRAV
  int MaxNumAMRCells; /*!< Maximum number of AMR cells per task */
  int MaxNumMultigridCells; /*!< Maximum number of AMR cells per task */
#endif
  ...
  int StandardCourantDiffusion;
  double MaxDiffusionTimeStep;
  double DiffusionCourantFac;
  double DiffusionCourantMinFac;
  double MetalsAmount[LT_NMet];
...
#ifdef PMGRID
  double AGS_MaxSoft, AGS_MinSoft;
#endif
#endif
...
/*This structure holds all of the information from the Sage files*/
extern struct sage_galaxies
{
  int   Type;
  int   FileNr;
...
/*This structure holds the necessary information from the Sage
 * galaxies to calculate the disk force and potential
 */

extern struct All_Gal_info
{
   float Mvir;       //SAGE galaxy data, halo Mvir
   float Pos[3];     //SAGE galaxy positions, halo center
   float Vel[3];     //SAGE galaxy velocity, sub halo velocity
   float Rvir;       //SAGE galaxy radius
   float DiskScaleRadius;
   float ColdGas;
   float StellarMass;
   float BulgeMass;
   float BlackHoleMass;
   long long mostbound;
   int subfnum;      //Subfind tab file number containing halo from merger tree
   int subhaloidx;   //Subhalo index from the merger tree file
   int MhaloIdx;
   unsigned int sub_len;      //Length of subhalo in single subfind tab file
   unsigned int sub_grnr;     //Group number that the subhalo is in single subfind tab file
   unsigned int group_firstsub;  //First sub in the group number for when all files are read in at once
   unsigned int group_len;       //Length of group
   unsigned int firstlen;
   unsigned int group_offset;        //Length of first halo in group
   unsigned int poffset;         //offset in id files when they are all read in at once
   float veldisp;        //subhalo veldisp
   float haloPos[3];
   float halovir;
   float anglez;
   float angley;
   ...
   float NewBulgeMass;
   float NewBlackHoleMass;
   float NewDiskScaleRadius;
   float coldslope;
   float coldb;
   float stellarslope;
   float stellarb;
...
extern struct Old_Gal_info
{
   float oldColdGas;
   float oldStellarMass;
   float oldBulgeMass;
   float oldBlackHoleMass;
   float oldDiskScaleRadius;
   float timestep;
   long long oldmostbound;
   float oldJ[3];
   float oldGalVel[3];
   float oldGalPos[3];
   unsigned int oldsub_len;
}*OldGal;

extern struct Rotation_R_vector
{
   float Rx;
   float Ry;
   float Rz;
...
extern struct New_P_Components
{
   float Px;
   float Py;
   float Pz;
...
extern struct Sage_Path_Names
{
    char paths[100];

}*sagepath;

extern struct Tree_Path_Names
{
    char tpaths[100];
}*treepath;


extern struct Halo_tree_read
{
  int Descendant;
  int FirstProgenitor;
  int NextProgenitor;
...
extern struct Halo_tree_header
{
  int haloNtrees;
  int Ntothalos;
}*halohead;

extern struct Halos_per_tree
{
   int halospertree;
}*hptree;


extern struct Subfind_data_tab
{
  ...
  unsigned int group_contamination_count;
  float group_contamination_mass;
  unsigned int group_nsubs;
  unsigned int group_firstsub;

  unsigned int sub_len;
  unsigned int sub_offset;
  unsigned int sub_parent;
  float sub_mass;
  float sub_pos[3];
  float sub_vel[3];
  float sub_cm[3];
  ...
  float sub_vmax;
  float sub_vmaxrad;
  float sub_halfmassrad;
  unsigned int sub_id_mostbound;
  unsigned int sub_grnr;

}*subfind_tab;

extern struct SubFind_Header_data  
{
    unsigned int ngroups;
    unsigned int totngroups;
    unsigned int nids;
    long long totnids;
    unsigned int ntask;
    unsigned int nsubs;
    unsigned int totnsubs;
}*SubFindHeader;

extern struct Subfind_multi_tab
{
...
extern struct SubFind_Header_multiple  
{
    unsigned int ngroups;
    unsigned int totngroups;
    unsigned int nids;
...
extern struct Subfind_ids
{
   unsigned int part_ids;

}*Subf_pids;

extern struct SubFind_Ids_Header
{
     unsigned int idngroups;
     unsigned int idtotngroups;
     unsigned int idnids;
     long long idtotnids;
     unsigned int idntask;
...
extern struct state_of_system
{
  double Mass,
    ...
    EnergyPot,
    EnergyInt,
#if defined(MAGNETIC_STATISTICS) || defined(FS_ALFA2_QUENCH)
    EnergyMag,
#endif
#if defined(FS_ALFA2_TURB) 
    DensestPart[4],
#endif
    EnergyTot,
    Momentum[4],
...
#ifdef AB_SHOCK_STATISTICS
  double MachHydroMass, MachHydroVol;
#if defined(MAGNETIC_STATISTICS) && !defined(AB_SHOCK_MAGNETIC)
  double MachMagMass, MachMagVol;
#endif
#endif
#if defined(MAGNETIC_STATISTICS) && defined(TRACEDIVB)
    double DivBerr;
#endif
#if defined(FS_ALFA2_QUENCH)
    double MaxAlfa2;
#endif
}
...
extern ALIGN(32) struct NODE
{
  ...
  int level;      /*!< starting with one for largest node */
  int amr_node;   /*!< 1 for lowest level nodes that have 7 siblings */
  int num_daughters;   /*!< number of regular daughter nodes  */
  
  MyFloat len_orig;
  MyFloat volume;
  
  int nxm1, nxp1, nym1, nyp1, nzm1, nzp1;   /*!< neighbor nodes on same or coarser grid level */
  int red;   /*!< flag for red-black sweep */
...
//stuff for allvars_bt.h
extern int  SnapshotNum;
extern long long totalpart;
...
extern struct halotree_catalogue
{
      int TotNsubhalos;
      int TotNgroups;
}
...
extern double time_total,time_read,time_prep,time_find,time_count,
       time_decide,time_write;

extern double time_readA,time_readB,time_readC;
extern double time_prepA,time_prepB,time_prepC;
extern double time_findAB,time_findBA,time_findBC,time_findAC;
extern double time_countAB,time_countBC,time_countAB_back;
extern double time_decide_desc,time_decide_back;


#define MASS1_STRING "MVIR"

#define MASS2_STRING "M5CC"
 
#define MASS3_STRING "M500"
...
//extern static unsigned long Nblocks = 0;
//extern static void *Table[MAXBLOCKS];
//extern static size_t SizeTable[MAXBLOCKS];
//extern static size_t TotMem = 0, HighMarkMem = 0, OldPrintedHighMark = 0;


extern struct GALAXY_OUTPUT  
{
  int   Type;
  int   FileNumber;
  long long   GalaxyIndex;
...
extern struct halo_aux_data_s   // auxiliary halo data 
{
  int DoneFlag;
  int HaloFlag;
  int NGalaxies;
...
extern double metallicities_sage[8];
...
extern int    FilesPerSnapshot_s; //Dont need just number of processors
extern int    LastSnapShotNr_s;   //Dont need just LastSnapShotNr

extern char   Sageparam[512];
extern char   OutputDir_s[512];
extern char   FileNameGalaxies[512];
extern char   SimulationDir_s[512];
//extern char   FileWithOutputSnaps_s[512];
//extern char   FileWithSnapList_s[512];

extern double Mass_DMpart;
extern int    TotHalos_s;
extern int    TotGalaxies[NOUT];
extern int    *TreeNgals[NOUT];

extern int    *FirstHaloInSnap_s;
...
extern double Omega;
extern double OmegaLambda_s;
extern double PartMass_s;
extern double Hubble_h_s;
extern double EnergySNcode, EnergySN_s;
...
/*UnitLength_in_cm,
  ...
  G,
  Hubble,    NOTE ADD ALL. FOR Parameter changes in sage source files
*/
...
enum Bucket
{
    Matchids = 0,  //set the first one to 0 so you can index into the array, compiler will assign
    createhash,
    perturb,
    missingpart,
    Galax,
    ...
    misspart1,
    forceadd,
    NUM_BUCKETS  //This stays at the end so you can always know how many buckets you have
};

extern struct ProfileData
{
   int procRank;
   double min_time;
   double max_time;
...
extern struct ProfileData gProfileData[NUM_BUCKETS];

struct ProfileInstance
{
   clock_t time;   //start() will fill this with start time, end() will subtract what's already here and store it.
   int bucketname;
   double timediff;
...
extern char tag_path[150];
extern char tag_file_name[200];
//#define f_mb	0.1
extern float f_mb;
#ifdef TAG_IN_HDF
extern hid_t Tfile_id;
#endif
// to keep tagged particle info
...
double infallMvir;
float LastMajorMerger;
}tagged_p;//,*tagged_list;
//to keep the tagged halo info
extern struct tagged_halo
{
int ID;
int Snap;
int HID;
