...
double metallicity_dependent_star_formation(int p)
{
  ...
  Sigma_c0 = Gal[p].ColdGas / (2.0 * M_PI * Gal[p].DiskScaleRadius * Gal[p].DiskScaleRadius) 
    * (All.UnitMass_in_g / (All.UnitLength_in_cm * All.UnitLength_in_cm)); // in g/cm^2
  ...
  return (Gal[p].ColdGas * 1.0e10) / (2.6 * 1.0e9) * SFR / (All.UnitMass_in_g / All.UnitTime_in_s * SEC_PER_YEAR / SOLAR_MASS);
